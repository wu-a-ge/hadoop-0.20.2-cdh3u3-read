...
public class DistCp implements Tool
{
	...
	static enum Options
	{
		DELETE("-delete", NAME + ".delete"), FILE_LIMIT("-filelimit", NAME
				+ ".limit.file"), SIZE_LIMIT("-sizelimit", NAME + ".limit.size"), IGNORE_READ_FAILURES(
				"-i", NAME + ".ignore.read.failures"), PRESERVE_STATUS("-p",
				NAME + ".preserve.status"), OVERWRITE("-overwrite", NAME
				+ ".overwrite.always"), UPDATE("-update", NAME
				+ ".overwrite.ifnewer"), SKIPCRC("-skipcrccheck", NAME
				+ ".skip.crc.check");
		...
		private long parseLong(String[] args, int offset)
		{
			...
			{
				throw new IllegalArgumentException("<n> not specified in "
						+ cmd);
			}
			long n = StringUtils.TraditionalBinaryPrefix
					.string2long(args[offset]);
			if (n <= 0)
			{
				throw new IllegalArgumentException("n = " + n + " <= 0 in "
						+ cmd);
			}
			return n;
		}
	...
	static final String PRESERVE_STATUS_LABEL = Options.PRESERVE_STATUS.propertyname
			+ ".value";
				...
				throws IOException
		{
			...
			if (cnfiles < 0 || cbsize < 0 || "".equals(srcfilelist))
			{
				throw new RuntimeException("Invalid metadata: #files("
						+ cnfiles + ") total_size(" + cbsize + ") listuri("
						+ srcfilelist + ")");
			}
			...
			try
			{
				...
				for (; sl.next(key, value); last = sl.getPosition())
				{
					...
					if (acc + key.get() > targetsize && acc != 0)
					{
						...
						splits.add(new FileSplit(src, pos, splitsize,
								(String[]) null));
						cbrem -= splitsize;
						pos = last;
						acc = 0L;
			...
			Mapper<LongWritable, FilePair, WritableComparable<?>, Text>
	{
		...
		{
			return update
					&& !sameFile(srcstatus.getPath().getFileSystem(job),
				...
				FileStatus srcstat) throws IOException
		{
			...
			FsPermission permission = preseved
					.contains(FileAttribute.PERMISSION) ? srcstat
					.getPermission() : null;
			short replication = preseved.contains(FileAttribute.REPLICATION) ? srcstat
					.getReplication()
					: destFileSys.getDefaultReplication();
			long blockSize = preseved.contains(FileAttribute.BLOCK_SIZE) ? srcstat
					.getBlockSize()
					: destFileSys.getDefaultBlockSize();
			return destFileSys.create(f, permission, true, sizeBuf,
				...
				Reporter reporter) throws IOException
		{
			...
			{
				// open src file
				in = srcstat.getPath().getFileSystem(job).open(
						srcstat.getPath());
				...
				for (int cbread; (cbread = in.read(buffer)) >= 0;)
				{
					...
					cbcopied += cbread;
					reporter.setStatus(String.format("%.2f ", cbcopied * 100.0
							/ srcstat.getLen())
							+ absdst
							+ " [ "
			...
			else
			{
				if (totfiles == 1)
				{
					...
					if (!(destFileSys.exists(dstparent) && destFileSys
				...
				if (dststat.getLen() != srcstat.getLen())
				{
					...
					throw new IOException("File size not matched: copied "
							+ bytesString(dststat.getLen()) + " to dst (="
							+ absdst + ") but expected "
							+ bytesString(srcstat.getLen()) + " from "
							+ srcstat.getPath());
				}
		...
		public void configure(JobConf job)
		{
			...
			catch (IOException ex)
			{
				throw new RuntimeException(
						"Unable to get the named file system.", ex);
			}
			sizeBuf = job.getInt("copy.buf.size", 128 * 1024);
			buffer = new byte[sizeBuf];
			ignoreReadFailures = job.getBoolean(
					Options.IGNORE_READ_FAILURES.propertyname, false);
			preserve_status = job.getBoolean(
					Options.PRESERVE_STATUS.propertyname, false);
			if (preserve_status)
			{
			...
			overwrite = !update
					&& job.getBoolean(Options.OVERWRITE.propertyname, false);
			...
			catch (IOException e)
			{
				...
				try
				{
					for (int i = 0; i < 3; ++i)
					{
						try
						{
							final Path tmp = new Path(job.get(TMP_DIR_LABEL),
									relativedst);
	...
	public static void copy(Configuration conf, String srcPath,
			String destPath, Path logPath, boolean srcAsList,
			boolean ignoreReadFailures) throws IOException
	{
		...
		EnumSet<Options> flags = ignoreReadFailures ? EnumSet
				.of(Options.IGNORE_READ_FAILURES) : EnumSet
			...
			throws IOException
	{
		...
		TokenCache.obtainTokensForNamenodes(jobConf.getCredentials(), ps,
				jobConf);
		...
		for (Path p : srcPaths)
		{
			...
			if (!fs.exists(p))
			{
				rslt.add(new IOException("Input source " + p
						+ " does not exist."));
			}
		}
		if (!rslt.isEmpty())
			...
			final Path destPath, String presevedAttributes) throws IOException
	{
		...
		EnumSet<FileAttribute> preseved = FileAttribute
				.parse(presevedAttributes);
		if (!preseved.contains(FileAttribute.USER)
				...
				throws IOException
		{
			...
			if (isDelete && !isOverwrite && !isUpdate)
			{
				throw new IllegalArgumentException(Options.DELETE.cmd
						+ " must be specified with " + Options.OVERWRITE
						+ " or " + Options.UPDATE + ".");
			}
			if (!isUpdate && skipCRC)
			{
				throw new IllegalArgumentException(Options.SKIPCRC.cmd
						+ " is relevant only with the " + Options.UPDATE.cmd
						+ " option");
			}
	...
	public int run(String[] args)
	{
		...
		{
			System.err
					.println("With failures, global counters are inaccurate; "
	...
	static String makeRelative(Path root, Path absPath)
	{
		...
		for (StringTokenizer rootTokens = new StringTokenizer(root.toUri()
				.getPath(), "/"); rootTokens.hasMoreTokens();)
		{
	...
	private static void setMapCount(long totalBytes, JobConf job)
			throws IOException
	{
		int numMaps = (int) (totalBytes / job.getLong(BYTES_PER_MAP_LABEL,
				BYTES_PER_MAP));
		numMaps = Math.min(numMaps, job.getInt(MAX_MAPS_LABEL,
				MAX_MAPS_PER_NODE
						* new JobClient(job).getClusterStatus()
								.getTaskTrackers()));
			...
			final Arguments args) throws IOException
	{
		...
		if (logPath == null)
		{
			...
			if (!dstExists || !dstIsDir)
			{
				...
				if (null == parent)
				{
					// If dst is '/' on S3, it might not exist yet, but
					// dst.getParent()
					// will return null. In this case, use '/' as its own parent
					// to prevent
					// NPE errors below.
					parent = args.dst;
				}
		...
		final boolean special = (args.srcs.size() == 1 && !dstExists) || update
				|| overwrite;
		...
		try
		{
			for (Iterator<Path> srcItr = args.srcs.iterator(); srcItr.hasNext();)
			{
				...
				Path root = special && srcfilestat.isDir() ? src : src
						.getParent();
				...
				for (pathstack.push(srcfilestat); !pathstack.empty();)
				{
					...
					for (int i = 0; i < children.length; i++)
					{
						...
						else
						{
							...
							// same.
							skipfile = update
									&& sameFile(srcfs, child, dstfs, new Path(
											args.dst, dst), skipCRCCheck);
							...
							if (!skipfile)
							{
								...
								if (cnsyncf > SYNC_FILE_MAX
										|| cbsyncs > maxBytesPerMap)
								{
									src_writer.sync();
									dst_writer.sync();
									cnsyncf = 0;
						...
						if (!skipfile)
						{
							src_writer.append(new LongWritable(
									child.isDir() ? 0 : child.getLen()),
									new FilePair(child, dst));
						}

						dst_writer.append(new Text(dst), new Text(child
								.getPath().toString()));
					}
					...
					if (cur.isDir())
					{
						...
						dir_writer
								.append(new Text(dst), new FilePair(cur, dst));
		...
		{
			deleteNonexisting(dstfs, dststatus, sorted, jobfs, jobDirectory,
					jobConf, conf);
		}

		Path tmpDir = new Path((dstExists && !dstIsDir)
				|| (!dstExists && srcCount == 1) ? args.dst.getParent()
			...
			throws IOException
	{
		...
		{
			/*
			 * Two possible cases: (1) src existed once but was deleted between
			 * the time period that srcstatus was obtained and the try block
			 * above. (2) srcfs does not support file checksum and (incorrectly)
			 * throws FNFE, e.g. some previous versions of HftpFileSystem. For
			 * case (1), it is okay to return true since src was already
			 * deleted. For case (2), true should be returned.
			 */
			return true;
		}
		...
		try
		{
			final FileChecksum dstcs = dstfs.getFileChecksum(dststatus
					.getPath());
			...
			Configuration conf) throws IOException
	{
		if (!dstroot.isDir())
		{
			throw new IOException("dst must be a directory when option "
					+ Options.DELETE.cmd + " is set, but dst (= "
		...
		final SequenceFile.Writer writer = SequenceFile.createWriter(jobfs,
				jobconf, dstlsr, Text.class, dstroot.getClass(),
				SequenceFile.CompressionType.NONE);
		try
		{
			...
			for (lsrstack.push(dstroot); !lsrstack.isEmpty();)
			{
				...
				if (status.isDir())
				{
					...
					{
						String relative = makeRelative(dstroot.getPath(), child
								.getPath());
		...
		try
		{
			...
			for (; lsrin.next(lsrpath, lsrstatus);)
			{
				...
				else
				{
					...
					String s = new Path(dstroot.getPath(), lsrpath.toString())
							.toString();
					if (shellargs[1] == null
							|| !isAncestorPath(shellargs[1], s))
					{
						shellargs[1] = s;
						int r = 0;
						try
						...
						if (r != 0)
						{
							throw new IOException("\"" + shellargs[0] + " "
									+ shellargs[1]
