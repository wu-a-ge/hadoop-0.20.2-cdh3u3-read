...
public class MRAsyncDiskService
{
	 ...
	 *            The roots of the file system volumes, which may be absolte
	 *            paths, or paths relative to the ${user.dir} system property
	 *            ("cwd").
	 */
		...
		for (int v = 0; v < volumes.length; v++)
		{
			...
			if (files != null)
			{
				for (int f = 0; f < files.length; f++)
				{
					...
					String absoluteFilename = files[f].getPath().toUri()
							.getPath();
					String relative = TOBEDELETED + Path.SEPARATOR_CHAR
							+ files[f].getPath().getName();
					DeleteTask task = new DeleteTask(volumes[v],
			...
			throws IOException
	{
		...
		try
		{
			if (!localFileSystem.rename(source, target))
			{
				...
				// This is necessary because rename can return false if the
				// source
				// does not exists.
				...
				// Try to recreate the parent directory just in case it gets
				// deleted.
				if (!localFileSystem.mkdirs(new Path(volume, TOBEDELETED)))
				{
					throw new IOException("Cannot create " + TOBEDELETED
							+ " under " + volume);
				}
	 ...
	 * 
	 * Note that if there is no volume that contains this path, the path will
	 * stay as it is, and the function will return false.
	 * 
	 * This functions returns when the moves are done, but not necessarily all
	 * deletions are done. This is usually good enough because applications
	 * won't see the path name under the old name anyway after the move.
