...
/**
 * Controls initialization, finalization and clean up of tasks, and also the
 * launching and killing of task JVMs.
 * 
 * This class defines the API for initializing, finalizing and cleaning up of
 * tasks, as also the launching and killing task JVMs. Subclasses of this class
 * will implement the logic required for performing the actual actions.
 * 
 * <br/>
 * 
...
public abstract class TaskController implements Configurable
{
	...
	final public static FsPermission TASK_LAUNCH_SCRIPT_PERMISSION = FsPermission
	 ...
	 * 
	 * @param allocator
	 *            the local dir allocator to use
	 * @param localStorage
	 *            TaskTracker's LocalStorage object
	 */
	 ...
	 * all of the job and private distributed cache files. Creates both the user
	 * directories and the job log directory.
	 * 
	 * @param user
	 *            the user name
	 * @param jobid
	 *            the job
	 * @param credentials
	 *            a filename containing the job secrets
	 * @param jobConf
	 *            the path to the localized configuration file
	 * @param taskTracker
	 *            the connection the task tracker
	 * @param ttAddr
	...
	public abstract void initializeJob(String user, String jobid,
			Path credentials, Path jobConf, TaskUmbilicalProtocol taskTracker,
	 ...
	 * @param attemptId
	 *            the attempt id (cleanup attempts have .cleanup suffix)
	 * @param setup
	 *            list of shell commands to execute before the jvm
	 * @param jvmArguments
	 *            list of jvm arguments
	 * @param currentWorkDirectory
	 *            the full path of the cwd for the task
	 * @param stdout
	 ...
	 * @throws IOException
	 */
	public abstract int launchTask(String user, String jobId, String attemptId,
	 ...
	 */
	public abstract void signalTask(String user, int taskPid, Signal signal)
			throws IOException;
	 ...
	 *            the user name
	 * @param subDir
	 *            the path relative to the user's subdirectory under the task
	 *            tracker root directories.
	 * @throws IOException
	 */
	public abstract void deleteAsUser(String user, String subDir)
			throws IOException;
	 ...
	 * @throws IOException
	 */
	public abstract void createLogDir(TaskAttemptID taskID, boolean isCleanup)
			throws IOException;
	 ...
	 * @throws IOException
	 */
	public abstract void deleteLogAsUser(String user, String subDir)
			throws IOException;
	...
	static class DeletionContext extends CleanupQueue.PathDeletionContext
	{
		...
		{
			return (isLog ? "log(" : "dir(") + user + "," + subDir + ")";
		}
	}
			...
			Path commandFile) throws IOException
	{
		...
		{
			LOG.error(
					"Caught IOException while writing JVM command line to file. ",
					ioe);
			throw ioe;
		}
		finally
