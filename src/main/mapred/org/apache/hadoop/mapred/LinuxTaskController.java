...
/**
 * A {@link TaskController} that runs the task JVMs as the user who submits the
 * job.
 * 
 * This class executes a setuid executable to implement methods of the
 * {@link TaskController}, including launching the task JVM and killing it when
 * needed, and also initializing and finalizing the task environment.
 * <p>
 * The setuid executable is launched using the command line:
 * </p>
 * <p>
 * task-controller user-name good-local-dirs command command-args, where
 * </p>
 * <p>
 * user-name is the name of the owner who submits the job
 * </p>
 * <p>
 * good-local-dirs is comma separated list of good mapred local dirs
 * </p>
 * <p>
 * command is one of the cardinal value of the
 * {@link LinuxTaskController.TaskControllerCommands} enumeration
 * </p>
 * <p>
 * command-args depends on the command being launched.
 * </p>
 * 
 * In addition to running and killing tasks, the class also sets up appropriate
 * access for the directories and files that will be used by the tasks.
 */
class LinuxTaskController extends TaskController
{
	...
	public void setConf(Configuration conf)
	{
		...
		String defaultTaskController = new File(nativeSbin, "task-controller")
				.getAbsolutePath();
	...
	enum Commands
	{
		INITIALIZE_JOB(0), LAUNCH_TASK_JVM(1), SIGNAL_TASK(2), DELETE_AS_USER(3), DELETE_LOG_AS_USER(
				4), RUN_COMMAND_AS_USER(5);
	...
	/**
	 * Result codes returned from the C task-controller. These must match the
	 * values in task-controller.h.
	 */
	enum ResultCode
	{
		OK(0), INVALID_USER_NAME(2), INVALID_TASK_PID(9), INVALID_CONFIG_FILE(
				24);
			...
			throws IOException
	{
		...
		// are also present.
		String[] taskControllerCmd = new String[] {
			taskControllerExe
		};
		...
		catch (ExitCodeException e)
		{
			...
			if (exitCode != 1)
			{
				LOG.warn("Exit code from checking binary permissions is : "
						+ exitCode);
			...
			InetSocketAddress ttAddr) throws IOException
	{
		List<String> command = new ArrayList<String>(Arrays.asList(
				taskControllerExe, user, localStorage.getDirsString(),
				Integer.toString(Commands.INITIALIZE_JOB.getValue()), jobid,
				credentials.toUri().getPath().toString(), jobConf.toUri()
						.getPath().toString()));
	...
	@Override
	public int launchTask(String user, String jobId, String attemptId,
			List<String> setup, List<String> jvmArguments,
			File currentWorkDirectory, String stdout, String stderr)
			throws IOException
	{
		...
		try
		{
			...
			Path p = new Path(allocator.getLocalPathForWrite(TaskTracker
					.getPrivateDirTaskScriptLocation(user, jobId, attemptId),
			...
			String[] command = new String[] {
					...
					jobId, attemptId, currentWorkDirectory.toString(),
					commandFile
			};
	...
	public void createLogDir(TaskAttemptID taskID, boolean isCleanup)
			throws IOException
	{
		// Log dirs are created during attempt dir creation when running the
		// task
	}
			...
			throws IOException
	{
		...
		catch (ExitCodeException e)
		{
			...
			{
				logOutput(shExec.getOutput());
				throw new IOException("Problem signalling task " + taskPid
						+ " with " + signal + "; exit = " + ret_code);
			}
		}
	}
			...
			throws IOException
	{
		...
		String taskRanFile = TaskTracker.TT_LOG_TMP_DIR + Path.SEPARATOR
				+ taskid;
		Configuration conf = getConf();

		// write the serialized task information to a file to pass to the
		// truncater
		Path taskRanFilePath = ldirAlloc
				.getLocalPathForWrite(taskRanFile, conf);
		...
		taskControllerCmd[3] = Integer.toString(Commands.RUN_COMMAND_AS_USER
				.getValue());
		...
		{
			LOG.warn("Exit code from " + taskControllerExe.toString()
					+ " is : " + shExec.getExitCode() + " for truncateLogs");
