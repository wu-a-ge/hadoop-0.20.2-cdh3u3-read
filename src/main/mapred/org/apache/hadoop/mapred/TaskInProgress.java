...
public class TaskInProgress
{
	...
	public TaskInProgress(JobID jobid, String jobFile, TaskSplitMetaInfo split,
			...
			int partition, int numSlotsRequired)
	{
	...
	/**
	 * Returns the type of the {@link TaskAttemptID} passed. The type of an
	 * attempt is determined by the nature of the task and not its id. For
	 * example, - Attempt 'attempt_123_01_m_01_0' might be a job-setup task even
	 * though it has a _m_ in its id. Hence the task type of this attempt is
	 * JOB_SETUP instead of MAP. - Similarly reduce attempt
	 * 'attempt_123_01_r_01_0' might have failed and is now supposed to do the
	 * task-level cleanup. In such a case this attempt will be of type
	 * TASK_CLEANUP instead of REDUCE.
	 */
	...
	synchronized boolean updateStatus(TaskStatus status)
	{
		...
		if (oldStatus != null)
		{
			...
			if ((newState == TaskStatus.State.RUNNING || newState == TaskStatus.State.UNASSIGNED)
					&& (oldState == TaskStatus.State.FAILED
							...
							|| oldState == TaskStatus.State.SUCCEEDED || oldState == TaskStatus.State.COMMIT_PENDING))
			{
				return false;
			}
		...
		else
		{
			taskStatuses.get(taskid).statusUpdate(status.getRunState(),
					status.getProgress(), status.getStateString(),
	...
	public void incompleteSubTask(TaskAttemptID taskid, JobStatus jobStatus)
	{
		...
		if (status != null)
		{
			...
			if (shouldFail != null)
			{
				...
				addDiagnosticInfo(taskid, "Task has been " + taskState
						+ " by the user");
			}
		...
		if (tasks.contains(taskid))
		{
			if (taskState == TaskStatus.State.FAILED)
			{
				...
				if (maxSkipRecords > 0)
				{
					...
					{
						LOG.debug("TaskInProgress adding"
								+ status.getNextRecordRange());
					}
		...
		{
			LOG.info("TaskInProgress " + getTIPId() + " has failed "
					+ numTaskFailures + " times.");
			kill();
		}
		this.user = job.getUser();
	}
	...
	{
		if (maxSkipRecords > 0
				&& numTaskFailures >= SkipBadRecords
	 ...
	 */
	private void completedTask(TaskAttemptID taskId,
			TaskStatus.State finalTaskState)
	{
		TaskStatus status = taskStatuses.get(taskId);
	...
	void alreadyCompletedTask(TaskAttemptID taskid)
	{
		...
		LOG.info("Already complete TIP " + getTIPId() + " has completed task "
				+ taskid);
	}
	...
	{
		return taskStatuses.values().toArray(
				new TaskStatus[taskStatuses.size()]);
	}
	...
	boolean killTask(TaskAttemptID taskId, boolean shouldFail)
	{
		...
		{
			String logStr = "Request received to "
					+ (shouldFail ? "fail" : "kill") + " task '" + taskId
					+ "' by user";
	...
	void recomputeProgress()
	{
		if (isComplete())
		{
			...
			TaskStatus completedStatus = taskStatuses
					.get(getSuccessfulTaskid());
		...
		else
		{
			...
			for (Iterator<TaskAttemptID> it = taskStatuses.keySet().iterator(); it
					.hasNext();)
			{
			...
			boolean taskCleanup)
	{
		...
		if (isMapTask())
		{
			...
			t = new MapTask(jobFile, taskid, partition,
					splitInfo.getSplitIndex(), numSlotsNeeded);
		}
		else
		{
	...
	private class FailedRanges
	{
		...
		synchronized void add(Range failedRange)
		{
			...
			if (maxSkipRecords == 0
					|| failedRange.getLength() <= maxSkipRecords)
			{
