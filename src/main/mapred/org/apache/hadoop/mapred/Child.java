...
class Child
{
	...
	static int getNumTasksToExecutePerJvm(JobConf job, boolean isMap)
	{
		int numTasksForJVM = job.getNumTasksToExecutePerJvm();
		if (isMap)
		{
			return job.getInt("mapred.job.map.reuse.jvm.num.tasks",
					numTasksForJVM);
		}
		else
		{
			return job.getInt("mapred.job.reduce.reuse.jvm.num.tasks",
					numTasksForJVM);
		}
	}

	static boolean logIsSegmented(JobConf job, boolean isMap)
	{
		return (getNumTasksToExecutePerJvm(job, isMap) != 1);
	}
	...
	public static void main(String[] args) throws Throwable
	{
		...
		final boolean isMap = firstTaskid.isMap();

		int jvmIdInt = Integer.parseInt(args[4]);
		JVMId jvmId = new JVMId(firstTaskid.getJobID(), firstTaskid.isMap(),
				jvmIdInt);
		...
		String jobTokenFile = System.getenv().get(
		...
		UserGroupInformation taskOwner = UserGroupInformation
				.createRemoteUser(firstTaskid.getJobID().toString());
		...
		try
		{
			while (true)
			{
				...
				currentJobSegmented = logIsSegmented(job, isMap);
				...
				TaskLog.syncLogs(logLocation, taskid, isCleanup,
						logIsSegmented(job, isMap));

				numTasksToExecute = getNumTasksToExecutePerJvm(job, isMap);
				LOG.info("numTasksToExecute = " + numTasksToExecute);
				...
				if (numTasksToExecute > 0
						&& ++numTasksExecuted == numTasksToExecute)
				{
					LOG.info("JVM Resue" + numTasksToExecute + ":"
							+ numTasksExecuted);
					break;
				}
			}
			...
			throws IOException
	{
		...
		Path localTaskFile = lDirAlloc.getLocalPathForWrite(
				TaskTracker.JOBFILE, jobConf);
