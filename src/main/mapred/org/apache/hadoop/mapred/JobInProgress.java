...
public class JobInProgress
{
	...
	private static final Comparator<TaskInProgress> failComparator = new Comparator<TaskInProgress>()
	{
		...
		public int compare(TaskInProgress t1, TaskInProgress t2)
		{
			...
			return (failures == 0) ? (t1.getTIPId().getId() - t2.getTIPId()
					.getId()) : failures;
		}
	};
	...
	/**
	 * Percentage of the cluster the job is willing to wait to get better
	 * locality
	 */
	...
	JobInProgress(JobTracker jobtracker, final JobConf default_conf,
			JobInfo jobInfo, int rCount, Credentials ts) throws IOException,
			InterruptedException
	{
		try
		{
			...
			String url = "http://" + jobtracker.getJobTrackerMachine() + ":"
					+ jobtracker.getInfoPort() + "/jobdetails.jsp?jobid="
					+ jobId;
			...
			Path submitJobFile = JobSubmissionFiles
					.getJobConfPath(jobSubmitDir);
			...
			{
				String desc = "The username " + conf.getUser()
						+ " obtained from the "
						+ "conf doesn't match the username " + user
			...
			String primaryGroup = (userGroups.length > 0) ? userGroups[0]
					: null;
			...
			this.profile = new JobProfile(user, jobId, jobFile, url,
					conf.getJobName(), conf.getQueueName());
			...
			status.setJobACLs(jobtracker.getJobACLsManager().constructJobACLs(
					conf));

			this.mapFailuresPercent = conf.getMaxMapTaskFailuresPercent();
			this.reduceFailuresPercent = conf.getMaxReduceTaskFailuresPercent();

			this.maxTaskFailuresPerTracker = conf
			...
			this.runningReduces = new LinkedHashSet<TaskInProgress>();
			this.resourceEstimator = new ResourceEstimator(this);
			this.reduce_input_limit = conf.getLong(
					"mapreduce.reduce.input.limit", DEFAULT_REDUCE_INPUT_LIMIT);
		...
		finally
		{
			...
			// At this point, this constructor is called in the context of an
			// RPC, and
	...
	/**
	 * Set the number of slots required to run a single map task-attempt. This
	 ...
	 * 
	 * @param slots
	 *            the number of slots required to run a single reduce
	 *            task-attempt
	 */
			...
			throws IOException
	{
		TaskSplitMetaInfo[] allTaskSplitMetaInfo = SplitMetaInfoReader
				.readSplitMetaInfo(jobId, fs, jobtracker.getConf(),
						jobSubmitDir);
			...
			TaskStatus status)
	{
		...
		if (change)
		{
			...
			TaskTrackerStatus ttStatus = (taskTracker == null) ? null
			...
			if (taskEvent != null)
			{
				...
				JobTrackerStatistics.TaskTrackerStat ttStat = jobtracker
						.getStatistics().getTaskTrackerStat(
								tip.machineWhereTaskRan(taskid));
				if (ttStat != null)
				{ // ttStat can be null in case of lost tracker
	...
	/**
	 * Returns map phase counters by summing over all map tasks in progress.
	 * This method returns true if counters are within limits and false
	 * otherwise.
	 */
	 ...
	 * the reduce counters. This method returns true if counters are within
			...
			throws IOException
	{
		...
		{
			// 修改使用
			result.setTaskRequireMemorySize(this.memoryPerMap);
			ResourceStatus rs = tts.getResourceStatus();
			rs.setCurrentOccupyMemMb(rs.getCurrentOccupyMemMb()
					+ result.getTaskRequireMemorySize());
			rs.setCurrentOccupyProcNum(rs.getCurrentOccupyProcNum()
					+ result.getNumSlotsRequired());
	 ...
	 *         <code>false</code> otherwise We check the number of missed
	 *         opportunities for the job. If it has 'waited' long enough we go
	 *         ahead and schedule.
	 */
			...
			int clusterSize, int numUniqueHosts) throws IOException
	{
		...
		/**
		 * check to see if we have any misbehaving reducers. If the expected
		 * output for reducers is huge then we just fail the job and error out.
		 * The estimated size is divided by 2 since the resource estimator
		 * returns the amount of disk space the that the reduce will use (which
		 * is 2 times the input, space for merge + reduce input).
		 **/
				...
				&& (reduce_input_limit > 0L))
		{
			...
			status.setFailureInfo("Job exceeded Reduce Input limit "
					+ " Limit:  " + reduce_input_limit + " Estimated: "
					+ estimatedReduceInputSize);
		...
		if (result != null)
		{
			result.setTaskRequireMemorySize(this.memoryPerReduce);
			ResourceStatus rs = tts.getResourceStatus();
			rs.setCurrentOccupyMemMb(rs.getCurrentOccupyMemMb()
					+ result.getTaskRequireMemorySize());
			rs.setCurrentOccupyProcNum(rs.getCurrentOccupyProcNum()
			...
			TaskType type, int numSlots)
	{
		...
		{
			jobtracker.getInstrumentation().addReservedReduceSlots(
					reservedSlots);
		}
			...
			TaskType type)
	{
		...
		if (type == TaskType.MAP)
		{
			jobtracker.getInstrumentation().decReservedMapSlots(
					info.getNumSlots());
		}
		else
		{
	...
	private synchronized void failMap(TaskInProgress tip)
	{
		...
		// Ignore locality for subsequent scheduling on this TIP. Always
		// schedule
	 ...
	 * 用于判断资源是否足够
	 * 
	 * @author yinzhaohui 2013-5-17 上午10:07:56
	 * @param ttStatus
	 * @param tip
	 * @return
	 */
	private boolean isTaskTrackerResourceLimited(TaskTrackerStatus ttStatus,
			TaskInProgress tip)
	{
		if(!tip.isJobSetupTask() && !tip.isJobCleanupTask())
		{
			ResourceStatus tt = ttStatus.getResourceStatus();
			if (tt.getTotalMemMb() != TaskTrackerStatus.UNAVAILABLE)
			{// tt.getTotalMemMb() == UNAVAILABLE 不判断内存
				long addMem = tip.isMapTask() ? this.memoryPerMap
						: this.memoryPerReduce;
				long requreTrackerMemMB = tt.getCurrentOccupyMemMb() + addMem;
				if (requreTrackerMemMB > tt.getTotalMemMb())
				{
					return true;
				}
			}
			if (tt.getTotalProcNum() != TaskTrackerStatus.UNAVAILABLE)
			{
				int addProcNum = tip.getNumSlotsRequired();
				long requreProcNum = tt.getCurrentOccupyProcNum() + addProcNum;
				if (requreProcNum > tt.getTotalProcNum())
				{
					return true;
				}
			}
			...
			int numUniqueHosts, boolean removeFailedTip)
	{
		...
		while (iter.hasNext())
		{
			...
			if (tip.isRunnable() && !tip.isRunning())
			{
				...
				{
					// 确定任务跟踪资源限制
					if (!isTaskTrackerResourceLimited(ttStatus, tip))
					{
			...
			double avgProgress, long currentTime, boolean shouldRemove)
	{
		...
		while (iter.hasNext())
		{
					...
					ttStatus.getTrackerName()))
			{
				...
				{
					// 确定任务跟踪资源限制
					if (!this.isTaskTrackerResourceLimited(ttStatus, tip))
					{
						// In case of shared list we don't remove it. Since the
						// TIP
						// failed
						// on this tracker can be scheduled on some other
						// tracker.
						if (shouldRemove)
						{
							iter.remove(); // this tracker is never going to run
											// it
											// again
						}
						return tip;
			...
			final int maxCacheLevel, final double avgProgress)
	{
		...
		// We fall to linear scan of the list ((3) above) if we have misses in
		// the
		// above caches

		// 0) Schedule the task with the most failures, unless failure was on
		// this
		// machine
		...
		if (node != null)
		{
			...
			// speculative) tasks or it might be NON_LOCAL_CACHE_LEVEL (i.e. -1)
			// if
	 ...
	 * 
	 * @param tip
	 *            {@link TaskInProgress} to be metered which just completed,
			...
			TaskStatus status)
	{
		...
		// Update jobhistory
		TaskTrackerStatus ttStatus = this.jobtracker
				.getTaskTrackerStatus(status.getTaskTracker());
		...
		else if (tip.isJobCleanupTask())
		{
			...
			jobtracker
					.markCompletedTaskAttempt(status.getTaskTracker(), taskid);
		}
		else if (tip.isMapTask())
		{
			runningMapTasks -= 1;
	...
	/**
	 * add up the counters and fail the job if it exceeds the counters. Make
	 * sure we do not recalculate the coutners after we fail the job. Currently
	 * this is taken care by terminateJob() since it does not calculate the
	 * counters.
	 */
	...
	private void jobComplete()
	{
		...
		if (this.status.getRunState() == JobStatus.RUNNING)
		{
			...
			JobHistory.JobInfo.logFinished(this.status.getJobID(), finishTime,
					...
					failedMapTasks, failedReduceTasks, mapCounters,
				...
				|| (status.getRunState() == JobStatus.PREP))
		{
			...
			if (jobTerminationState == JobStatus.FAILED)
			{
				...
				JobSummary.logJobSummary(this,
						jobtracker.getClusterStatus(false));
	...
	private void cancelReservedSlots()
	{
		...
		Set<TaskTracker> tm = new HashSet<TaskTracker>(
				trackersReservedForMaps.keySet());
		for (TaskTracker tt : tm)
		{
			tt.unreserveSlots(TaskType.MAP, this);
		}

		Set<TaskTracker> tr = new HashSet<TaskTracker>(
				trackersReservedForReduces.keySet());
			...
			boolean wasComplete, boolean wasAttemptRunning)
	{
		...
		if (wasAttemptRunning)
		{
			...
			// new map task attempt or reduce task attempt.We do not really
			// check
			// for tip being running.
			// Whenever we obtain new task attempt following counters are
			// incremented.
			// ++runningMapTasks;
			// .........
			// metrics.launchMap(id);
			...
			String trackerName)
	{
		TaskStatus status = TaskStatus.createTaskStatus(tip.isMapTask(),
				taskid, 0.0f, tip.isMapTask() ? numSlotsPerMap
						: numSlotsPerReduce, state, reason, reason,
		...
		boolean isComplete = tip.isComplete();
		if (wasComplete && !isComplete)
		{ // mark a successful tip as failed
			String taskType = getTaskType(tip);
	...
	public synchronized TaskInProgress getTaskInProgress(TaskID tipid)
	{
		if (tipid.isMap())
		{
			if (tipid.equals(cleanup[0].getTIPId()))
			{ // cleanup map tip
				return cleanup[0];
			}
			if (tipid.equals(setup[0].getTIPId()))
			{ // setup map tip
				return setup[0];
			}
			for (int i = 0; i < maps.length; i++)
		...
		{
			if (tipid.equals(cleanup[1].getTIPId()))
			{ // cleanup reduce tip
				return cleanup[1];
			}
			if (tipid.equals(setup[1].getTIPId()))
			{ // setup reduce tip
				return setup[1];
			}
			for (int i = 0; i < reduces.length; i++)
	 ...
	 * if it has data on the same rack, etc (depending on number of levels in
	...
	static class JobSummary
	{
		...
		static class SummaryBuilder
		{
			...
			{
				return _add(key, StringUtils.escapeString(
						String.valueOf(value), StringUtils.ESCAPE_CHAR,
						charsToEscape));
			}
		...
		static SummaryBuilder getTaskLaunchTimesSummary(JobInProgress job)
		{
			...
			synchronized (timeMap)
			{
				...
				{
					summary.add(
							"first" + StringUtils.camelize(e.getKey().name())
									+ "TaskLaunchTime", e.getValue()
									.longValue());
				}
			}
			return summary;
		 ...
		 * 
		 * @param job
		 *            {@link JobInProgress} whose summary is to be logged,
		 *            cannot be <code>null</code>.
		 * @param cluster
		 *            {@link ClusterStatus} of the cluster on which the job was
				...
				ClusterStatus cluster)
		{
			...
			long mapSlotSeconds = (jobCounters
					.getCounter(Counter.SLOTS_MILLIS_MAPS) + jobCounters
	...
	private void generateAndStoreTokens() throws IOException
	{
		...
		JobTokenIdentifier identifier = new JobTokenIdentifier(new Text(
				jobId.toString()));
		Token<JobTokenIdentifier> token = new Token<JobTokenIdentifier>(
				identifier, jobtracker.getJobTokenSecretManager());
