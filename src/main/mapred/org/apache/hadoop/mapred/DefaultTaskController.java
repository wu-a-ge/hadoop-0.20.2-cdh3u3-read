 ...
 * 
 * This class provides an implementation for launching and killing tasks that
 * need to be run as the tasktracker itself. Hence, many of the initializing or
 * cleanup methods are not required here.
 * 
 * <br/>
 * 
...
public class DefaultTaskController extends TaskController
{
	...
	private static final Log LOG = LogFactory
			.getLog(DefaultTaskController.class);
			...
			throws IOException
	{
		...
		try
		{
			...
			new Localizer(localFs, getConf().getTrimmedStrings(
					JobConf.MAPRED_LOCAL_DIR_PROPERTY)).initializeAttemptDirs(
					user, jobId, attemptId);
			...
			String logLocation = TaskLog.getAttemptDir(jobId, attemptId)
					.toString();
			...
			try
			{
				rawFs.setPermission(p,
						TaskController.TASK_LAUNCH_SCRIPT_PERMISSION);
			}
			catch (ExitCodeException ece)
			{
				// we don't want to confuse this exception with an
				// ExitCodeException
	 ...
	 * </ul>
	 * This method must be invoked in the access control context of the job
	 * owner user. This is because the distributed cache is also setup here and
	 * the access to the hdfs files requires authentication tokens in case where
	 * security is enabled.
	 * 
	 * @param user
	 *            the user in question (the job owner)
	 * @param jobid
	 ...
	 *            downloaded
	 * @param taskTracker
	 *            the connection to the task tracker
	 * @throws IOException
			...
			InetSocketAddress ttAddr) throws IOException, InterruptedException
	{
		...
		FileUtil.copy(localFs, credentials, localFs, localJobTokenFile, false,
				getConf());
		...
		localizer.localizeJobFiles(JobID.forName(jobid), jConf,
	...
	public void deleteLogAsUser(String user, String subDir) throws IOException
	{
		...
		for (String localdir : localStorage.getDirs())
		{
			...
			catch (Exception e)
			{
				...
				LOG.warn("Could not delete dir: " + dirPath + " , Reason : "
						+ e.getMessage());
			}
		}
	}
			...
			throws IOException
	{
		...
		trunc.truncateLogs(new JVMInfo(TaskLog.getAttemptDir(
				firstTask.getTaskID(), firstTask.isTaskCleanupTask()),
				allAttempts));
	}
