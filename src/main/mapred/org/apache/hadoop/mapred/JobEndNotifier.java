...
{
	private static final Log LOG = LogFactory.getLog(JobEndNotifier.class
			.getName());
	...
	public static void startNotifier()
	{
		...
		thread = new Thread(new Runnable()
		{
			...
			private void sendNotification(JobEndStatusInfo notification)
			{
				...
				{
					LOG.error("Notification failure [" + notification + "]",
							ioex);
					if (notification.configureForRetry())
					{
						...
						{
							LOG.error("Notification queuing error ["
									+ notification + "]", iex);
						}
					}
				}
		...
		if (uri != null)
		{
			...
			if (uri.contains("$jobStatus"))
			{
				String statusStr = (status.getRunState() == JobStatus.SUCCEEDED) ? "SUCCEEDED"
						: (status.getRunState() == JobStatus.FAILED) ? "FAILED"
								: "KILLED";
				uri = uri.replace("$jobStatus", statusStr);
			}
			notification = new JobEndStatusInfo(uri, retryAttempts,
					retryInterval);
		}
		return notification;
	}
	...
	public static void registerNotification(JobConf jobConf, JobStatus status)
	{
		...
		if (notification != null)
		{
			...
			{
				LOG.error(
						"Notification queuing failure [" + notification + "]",
						iex);
			}
		}
	}
	...
	public static void localRunnerNotification(JobConf conf, JobStatus status)
	{
		...
		if (notification != null)
		{
			while (notification.configureForRetry())
			{
				...
				{
					LOG.error("Notification error [" + notification.getUri()
							+ "]", ioex);
				}
				catch (Exception ex)
				{
				...
				try
				{
					synchronized (Thread.currentThread())
					{
						Thread.currentThread().sleep(
								notification.getRetryInterval());
					}
				}
				catch (InterruptedException iex)
				{
					LOG.error(
							"Notification retry error [" + notification + "]",
							iex);
				}
			}
		}
