...
public class Counters implements Writable, Iterable<Counters.Group>
{
	...
	private static char[] charsToEscape = {
			GROUP_OPEN, GROUP_CLOSE, COUNTER_OPEN, COUNTER_CLOSE, UNIT_OPEN,
			UNIT_CLOSE
	};
	...
	/** limit on counters **/
	public static int MAX_COUNTER_LIMIT = conf.getInt(
	...
	public static class Counter extends org.apache.hadoop.mapreduce.Counter
	{
		...
		public synchronized String makeEscapedCompactString()
		{
			...
			// First up, obtain the strings that need escaping. This will help
			// us
	...
	public class Group implements Writable, Iterable<Counter>
	{
		...
		public synchronized boolean equals(Object obj)
		{
			...
			if (obj != null && obj instanceof Group)
			{
				...
				{
					isEqual = true;
					for (Map.Entry<String, Counter> entry : subcounters
							.entrySet())
					{
		...
		public synchronized long getCounter(String counterName)
		{
			...
			{
				if (counter != null
						&& counter.getDisplayName().equals(counterName))
				{
		...
		public synchronized Counter getCounterForName(String name)
		{
			...
			if (result == null)
			{
				...
				numCounters = (numCounters == 0) ? Counters.this.size()
						: numCounters;
				if (numCounters >= MAX_COUNTER_LIMIT)
				{
					throw new CountersExceededException(
							"Error: Exceeded limits on number of counters - "
									+ "Counters=" + numCounters + " Limit="
									+ MAX_COUNTER_LIMIT);
				}
				result = new Counter(shortName, localize(shortName + ".name",
						shortName), 0L);
	 ...
	 *            amount by which counter is to be incremented
	 */
	public synchronized void incrCounter(String group, String counter,
			long amount)
	{
		Group retGroup = getGroup(group);
		if (retGroup != null)
	...
	public synchronized void incrAllCounters(Counters other)
	{
		for (Group otherGroup : other)
		{
			...
			{
				Counter counter = group.getCounterForName(otherCounter
						.getName());
				if (counter == null)
				{
					continue;
	 ...
	 * 
	 * @param name
	 *            the name of a group or counter
	 * @param limit
	...
	{
		return (name.length() > limit ? name.substring(name.length() - limit,
				name.length()) : name);
	}
			...
			throws ParseException
	{
		...
		while (groupString != null)
		{
			...
			String groupDisplayName = getBlock(groupString, UNIT_OPEN,
					UNIT_CLOSE, groupIndex);
			...
			String counterString = getBlock(groupString, COUNTER_OPEN,
					COUNTER_CLOSE, groupIndex);
			...
			while (counterString != null)
			{
				...
				// Get the actual name
				String counterName = getBlock(counterString, UNIT_OPEN,
				...
				counterString = getBlock(groupString, COUNTER_OPEN,
						COUNTER_CLOSE, groupIndex);
			}

			groupString = getBlock(compactString, GROUP_OPEN, GROUP_CLOSE,
					index);
		}
		return counters;
	}
