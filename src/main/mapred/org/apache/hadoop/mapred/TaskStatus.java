...
public abstract class TaskStatus implements Writable, Cloneable
{
	...
	/**
	 * Testcases can override {@link #getMaxStringSize()} to control the
	 * max-size of strings in {@link TaskStatus}. Note that the
	 * {@link TaskStatus} is never exposed to clients or users (i.e Map or
	 * Reduce) and hence users cannot override this api to pass large strings in
	 * {@link TaskStatus}.
	 */
	...
	public void setDiagnosticInfo(String info)
	{
		...
		diagnosticInfo = ((diagnosticInfo == null) ? info : diagnosticInfo
				.concat(info));
	...
	public void setStateString(String stateString)
	{
		if (stateString != null)
		{
			...
			{
				// log it
				LOG.info("state-string for task " + taskid + " : "
						+ stateString);
	...
	/**
	 * Sets finishTime for the task status if and only if the start time is set
	 * and passed finish time is greater than zero.
	 * 
	 * @param finishTime
	 *            finish time of task.
	 */
	...
	static TaskStatus createTaskStatus(DataInput in, TaskAttemptID taskId,
			float progress, int numSlots, State runState,
			String diagnosticInfo, String stateString, String taskTracker,
			Phase phase, Counters counters) throws IOException
	{
		boolean isMap = in.readBoolean();
		return createTaskStatus(isMap, taskId, progress, numSlots, runState,
				diagnosticInfo, stateString, taskTracker, phase, counters);
	}
	...
	{
		return (isMap) ? new MapTaskStatus(taskId, progress, numSlots,
				runState, diagnosticInfo, stateString, taskTracker, phase,
				counters) : new ReduceTaskStatus(taskId, progress, numSlots,
				runState, diagnosticInfo, stateString, taskTracker, phase,
				counters);
	}
