...
abstract class TaskRunner extends Thread
{
	...
	static final String MAPREDUCE_USER_CLASSPATH_FIRST = "mapreduce.user.classpath.first"; // a
																							// semi-hidden
																							// config
	 ...
	 * @param jobConf
	 *            job configuration
	 * @return the maximum virtual memory of the child task or <code>-1</code>
	...
	public final void run()
	{
		...
		try
		{
			...
			final LocalDirAllocator lDirAlloc = new LocalDirAllocator(
					"mapred.local.dir");
			// simply get the location of the workDir and pass it to the child.
			// The
			// child will do the actual dir creation
			final File workDir = new File(new Path(
					localdirs[rand.nextInt(localdirs.length)],
					TaskTracker.getTaskWorkDir(t.getUser(), taskid.getJobID()
							.toString(), taskid.toString(), t
							.isTaskCleanupTask())).toString());

			// Set up the child task's configuration. After this call, no
			// localization
			// of files should happen in the TaskTracker's process space. Any
			// changes to
			...
			Vector<String> vargs = getVMArgs(taskid, workDir, classPaths,
					logSize);
			...
			tracker.getTaskTrackerInstrumentation().reportTaskLaunch(taskid,
					stdout, stderr);
			...
			if (exitCodeSet)
			{
				if (!killed && exitCode != 0)
				{
					if (exitCode == 65)
					{
						tracker.getTaskTrackerInstrumentation().taskFailedPing(
								t.getTaskID());
					}
					throw new IOException(
							"Task process exit with nonzero status of "
									+ exitCode + ".");
				}
			}
		}
		...
		catch (Throwable throwable)
		{
			...
			try
			{
				tracker.internalReportDiagnosticInfo(t.getTaskID(),
						baos.toString());
			}
			catch (IOException e)
			{
		...
		finally
		{
			...
			// It is safe to call TaskTracker.TaskInProgress.reportTaskFinished
			// with
	...
	{
		jvmManager.launchJvm(this, jvmManager.constructJvmEnv(setup, vargs,
	 ...
	 * @param taskid
	 * @param isCleanup
	 * @return an array of files. The first file is stdout, the second is
	 *         stderr.
	 * @throws IOException
	 */
	...
	/**
	 * Write the child's configuration to the disk and set it in configuration
	 * so that the child can pick it up from there.
	 * 
	 * @param lDirAlloc
	 * @throws IOException
	 */
			...
			throws IOException
	{
		...
		Path localTaskFile = lDirAlloc.getLocalPathForWrite(TaskTracker
				.getTaskConfFile(t.getUser(), t.getJobID().toString(), t
						.getTaskID().toString(), t.isTaskCleanupTask()), conf);

		// write the child's task configuration file to the local disk
		JobLocalizer.writeLocalJobFile(localTaskFile, conf);

		// Set the final job file in the task. The child needs to know the
		// correct
	 ...
	 *            The string to parse
	 * @param taskid
	 *            The task ID to replace the special string with
	...
	private Vector<String> getVMArgs(TaskAttemptID taskid, File workDir,
			List<String> classPaths, long logSize) throws IOException
	{
		...
		//
		// The following symbols if present in
		...
		// </property>
		//
		String[] javaOptsSplit = parseChildJavaOpts(
				getChildJavaOpts(conf, JobConf.DEFAULT_MAPRED_TASK_JAVA_OPTS),
				taskid);
		...
		{
			if (conf.getProfileTaskRange(t.isMapTask()).isIncluded(
					t.getPartition()))
			{
				File prof = TaskLog.getTaskLogFile(taskid,
						t.isTaskCleanupTask(), TaskLog.LogName.PROFILE);
		...
		vargs.add(TaskLog.getAttemptDir(taskid, t.isTaskCleanupTask())
				.toString());
		return vargs;
	}

	private void setupLog4jProperties(Vector<String> vargs,
	 ...
	 * @throws IOException
	 */
	static Path createChildTmpDir(File workDir, JobConf conf, boolean createDir)
			throws IOException
	{
   ...
   */
	...
	private String getVMEnvironment(String errorInfo, File workDir,
			JobConf conf, Map<String, String> env, TaskAttemptID taskid,
			long logSize) throws Throwable
	{
		...
		hadoopClientOpts = hadoopClientOpts + "-Dhadoop.tasklog.taskid="
				+ taskid + " -Dhadoop.tasklog.iscleanup="
				+ t.isTaskCleanupTask() + " -Dhadoop.tasklog.totalLogFileSize="
				+ logSize;
		...
		if (mapredChildEnv != null && mapredChildEnv.length() > 0)
		{
			...
			for (String cEnv : childEnvs)
			{
				try
				{
					...
					{
						// this key is not configured by the tt for the child ..
						// get it
						// from the tt's env
						...
						{
							// the env key is note present anywhere .. simply
							// set it
	...
	static void setupChildMapredLocalDirs(Task t, JobConf conf)
	{
		...
		for (int i = 1; i < localDirs.length; i++)
		{
			childMapredLocalDir.append(","
					+ localDirs[i]
					+ Path.SEPARATOR
					+ TaskTracker.getLocalTaskDir(user, jobId, taskId,
							isCleanup));
		}
	 ...
	 * 
	 * @param jobJar
	 *            Job jar from configuration
	 * @param classPaths
	 *            Accumulator for class paths
	 */
	 ...
	 * Note that when we setup the distributed cache, we didn't create the
	 * symlinks. This is done on a per task basis by the currently executing
	 * task.
	 * 
	 * @param conf
	 *            The job configuration.
	 * @param workDir
	 *            Working directory, which is completely deleted.
	 */
	public static void setupWorkDir(JobConf conf, File workDir)
			throws IOException
	{
		...
		if (conf.getJar() != null)
		{
			File jobCacheDir = new File(new Path(conf.getJar()).getParent()
					.toString());
			...
			try
			{
				TrackerDistributedCacheManager.createAllSymlink(conf,
						jobCacheDir, workDir);
			}
			catch (IOException ie)
			{
