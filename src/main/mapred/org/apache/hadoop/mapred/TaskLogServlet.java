...
public class TaskLogServlet extends HttpServlet
{
			...
			TaskLog.LogName filter, boolean isCleanup) throws IOException
	{
		...
		try
		{
			InputStream taskLogReader = new TaskLog.Reader(taskId, filter,
					start, end, isCleanup);
	...
	/**
	 * Validates if the given user has job view permissions for this job. conf
	 * contains jobOwner and job-view-ACLs. We allow jobOwner, superUser(i.e.
	 * mrOwner) and cluster administrators and users and groups specified in
	 * configuration using mapreduce.job.acl-view-job to view job.
	 */
	private void checkAccessForTaskLogs(JobConf conf, String user,
			String jobId, TaskTracker tracker) throws AccessControlException
	{
		...
		AccessControlList queueAdminsACL = new AccessControlList(conf.get(
				QueueManager.toFullPropertyName(queue,
		...
		// check if user is queue admin or cluster admin or jobOwner or member
		// of
		// job-view-acl
	...
	/**
	 * Builds a JobConf object by reading the job-acls.xml file. This doesn't
	 * load the default resources.
	 * 
	 * Returns null if job-acls.xml is not there in userlogs/$jobid on local
	 * file system. This can happen when we restart the cluster with job level
	 * authorization enabled(but was disabled on earlier cluster) and viewing
	 * task logs of old jobs(i.e. jobs finished on earlier unsecure cluster).
	 */
			...
			throws ServletException, IOException
	{
		...
		if (user != null)
		{
			...
			TaskTracker taskTracker = (TaskTracker) context
			...
			if (jobACLConf != null)
			{
				try
				{
					checkAccessForTaskLogs(jobACLConf, user, jobId.toString(),
							taskTracker);
				}
				catch (AccessControlException e)
				{
					String errMsg = "User " + user
							+ " failed to view tasklogs of job " + jobId
							+ "!\n\n" + e.getMessage();
		...
		if (!plainText)
		{
			...
			if (filter == null)
			{
				...
				{
					printTaskLog(response, out, attemptId, start, end,
							plainText, TaskLog.LogName.SYSLOG, isCleanup);
				}
				if (haveTaskLog(attemptId, isCleanup, TaskLog.LogName.DEBUGOUT))
				{
					printTaskLog(response, out, attemptId, start, end,
							plainText, TaskLog.LogName.DEBUGOUT, isCleanup);
				}
				if (haveTaskLog(attemptId, isCleanup, TaskLog.LogName.PROFILE))
				{
					printTaskLog(response, out, attemptId, start, end,
							plainText, TaskLog.LogName.PROFILE, isCleanup);
				}
			}
			else
			{
