...
public class TaskTrackerStatus implements Writable
{
	...
	static
	{ // register a ctor
		WritableFactories.setFactory(TaskTrackerStatus.class,
				new WritableFactory()
				{
	...
	public static class ResourceStatus implements Writable
	{
		...
		// 增加相关参数
		private long totalMemMb = UNAVAILABLE;
		private int totalProcNum = UNAVAILABLE;
		private long currentOccupyMemMb = 0;
		private int currentOccupyProcNum = 0;
		...
		void setTotalMemMb(long totalMemMb)
		{
		...
		{
			return this.totalMemMb;
		}

		void setCurrentOccupyMemMb(long currentOccupyMemMb)
		{
		...
		void setTotalProcNum(int totalProcNum)
		{
		...
		{
			return this.totalProcNum;
		}

		void setCurrentOccupyProcNum(int currentOccupyProcNum)
		{
		 ...
		 * 
		 * If this is {@link JobConf#DISABLED_MEMORY_LIMIT}, it should be
		 ...
		 * 
		 * @param totalRAM
		 *            maximum amount of physical memory on the tasktracker in
		 *            bytes.
		 */
		...
		/**
		 * Set the memory size of each map slot on this TT. This will be used by
		...
		/**
		 * Set the memory size of each reduce slot on this TT. This will be used
		 * by JT for accounting more slots for jobs that use more memory.
		 * 
		 * @param mem
		 */
		...
		/**
		 * Set the amount of available virtual memory on the tasktracker. If the
		 * input is not a valid number, it will be set to UNAVAILABLE
		 * 
		 * @param vmem
		 *            amount of available virtual memory on the tasktracker in
		 *            bytes.
		 */
		void setAvailableVirtualMemory(long availableMem)
		{
			availableVirtualMemory = availableMem > 0 ? availableMem
					: UNAVAILABLE;
		}

		/**
		 * Get the amount of available virtual memory on the tasktracker. Will
		 * return UNAVAILABLE if it cannot be obtained
		 * 
		 * @return the amount of available virtual memory on the tasktracker in
		 *         bytes.
		 */
		...
		/**
		 * Set the amount of available physical memory on the tasktracker. If
		 * the input is not a valid number, it will be set to UNAVAILABLE
		 * 
		 * @param availableRAM
		 *            amount of available physical memory on the tasktracker in
		 *            bytes.
		 */
		void setAvailablePhysicalMemory(long availableRAM)
		{
			availablePhysicalMemory = availableRAM > 0 ? availableRAM
					: UNAVAILABLE;
		}

		/**
		 * Get the amount of available physical memory on the tasktracker. Will
		 * return UNAVAILABLE if it cannot be obtained
		 * 
		 * @return amount of available physical memory on the tasktracker in
		 *         bytes.
		 */
		...
		/**
		 * Set the CPU frequency of this TaskTracker If the input is not a valid
		 * number, it will be set to UNAVAILABLE
		 * 
		 * @param cpuFrequency
		...
		/**
		 * Get the CPU frequency of this TaskTracker Will return UNAVAILABLE if
		...
		/**
		 * Set the number of processors on this TaskTracker If the input is not
		 * a valid number, it will be set to UNAVAILABLE
		 * 
		 * @param numProcessors
		...
		{
			this.numProcessors = numProcessors > 0 ? numProcessors
					: UNAVAILABLE;
		}

		/**
		 * Get the number of processors on this TaskTracker Will return
		...
		/**
		 * Set the cumulative CPU time on this TaskTracker since it is up It can
		 * be set to UNAVAILABLE if it is currently unavailable.
		 * 
		 * @param cumulativeCpuTime
		 *            Used CPU time in millisecond
		 */
		public void setCumulativeCpuTime(long cumulativeCpuTime)
		{
			this.cumulativeCpuTime = cumulativeCpuTime > 0 ? cumulativeCpuTime
					: UNAVAILABLE;
		}

		/**
		 * Get the cumulative CPU time on this TaskTracker since it is up Will
		 * return UNAVAILABLE if it cannot be obtained
		 * 
		...
		/**
		 * Get the CPU usage on this TaskTracker Will return UNAVAILABLE if it
		 * cannot be obtained
		 * 
		 * @return CPU usage in %
		 */
		...
		public void write(DataOutput out) throws IOException
		{
			...
			WritableUtils.writeVLong(out, totalMemMb);
			WritableUtils.writeVInt(out, totalProcNum);
			WritableUtils.writeVLong(out, currentOccupyMemMb);
			WritableUtils.writeVInt(out, currentOccupyProcNum);
		}
		...
		public void readFields(DataInput in) throws IOException
		{
			...
			totalMemMb = WritableUtils.readVLong(in);
			totalProcNum = WritableUtils.readVInt(in);
			currentOccupyMemMb = WritableUtils.readVLong(in);
			currentOccupyProcNum = WritableUtils.readVInt(in);
		}
	}
   ...
   */
	...
	/**
   */
	public TaskTrackerStatus(String trackerName, String host, int httpPort,
			List<TaskStatus> taskReports, int taskFailures, int dirFailures,
   ...
   */
	...
	private boolean isTaskRunning(TaskStatus taskStatus)
	{
		...
		return (state == State.RUNNING || state == State.UNASSIGNED || taskStatus
				.inTaskCleanupPhase());
	}
   ...
   */
	...
	/**
	 * Static class which encapsulates the Node health related fields.
	 * 
	 */
	/**
	 * Static class which encapsulates the Node health related fields.
	...
	static class TaskTrackerHealthStatus implements Writable
	{
		...
		public TaskTrackerHealthStatus(boolean isNodeHealthy,
				String healthReport, long lastReported)
		{
		...
		/**
		 * Sets when the TT got its health information last from node health
		 * monitoring service.
		 * 
		 * @param lastReported
		 *            last reported time by node health script
		 */
