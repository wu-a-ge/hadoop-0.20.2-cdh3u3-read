...
class NodeHealthCheckerService
{
	...
	/**
	 * Class which is used by the {@link Timer} class to periodically execute
	 * the node health script.
	 * 
	 */
	private class NodeHealthMonitorExecutor extends TimerTask
	{
		...
		public NodeHealthMonitorExecutor(String[] args)
		{
			...
			shexec = new ShellCommandExecutor(
					(String[]) execScript
							.toArray(new String[execScript.size()]),
					null, null, scriptTimeout);
		}
		 ...
		 * <li>The node health scripts output has a line which begins with ERROR
		 * </li>
		 * <li>An exception is thrown while executing the script</li>
		 * </ol>
		 * If the script throws {@link IOException} or {@link ExitCodeException}
		 * the output is ignored and node is left remaining healthy, as script
		 * might have syntax error.
		 * 
		 * @param status
		 */
		...
		/**
		 * Method to check if the output string has line which begins with
		 * ERROR.
		 * 
		 * @param output
		 *            string
	...
	/*
	 * Method which initializes the values for the script path and interval
	 * time.
	 */
	...
	/**
	 * Returns output from health script. if node is healthy then an empty
	 * string is returned.
	 * 
	...
	/**
	 * Method used to determine if or not node health monitoring service should
	 * be started or not. Returns true if following conditions are met:
	 * 
	 * <ol>
	...
	/**
	 * Test method to directly access the timer which node health checker would
	 * use.
	 * 
	 * 
	 * @return Timer task
