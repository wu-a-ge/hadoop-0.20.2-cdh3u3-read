...
class JobQueueTaskScheduler extends TaskScheduler
{
	...
	public static final Log LOG = LogFactory
			.getLog(JobQueueTaskScheduler.class);
			...
			throws IOException
	{
		...
		scheduleMaps: for (int i = 0; i < availableMapSlots; ++i)
		{
			synchronized (jobQueue)
			{
				for (JobInProgress job : jobQueue)
				{
					...
					t = job.obtainNewLocalMapTask(taskTrackerStatus,
							numTaskTrackers,
		...
		if (availableReduceSlots > 0)
		{
			...
			synchronized (jobQueue)
			{
				for (JobInProgress job : jobQueue)
				{
					...
					Task t = job.obtainNewReduceTask(taskTrackerStatus,
							numTaskTrackers,
							taskTrackerManager.getNumberOfUniqueHosts());
					if (t != null)
					{
		...
		{
			LOG.debug("Task assignments for "
