 ...
 * Creates the following hierarchy: <li>$mapred.local.dir/taskTracker/$user</li>
 * <li>$mapred.local.dir/taskTracker/$user/jobcache</li> <li>
 * $mapred.local.dir/taskTracker/$user/jobcache/$jobid/work</li> <li>
 * $mapred.local.dir/taskTracker/$user/jobcache/$jobid/jars</li> <li>
 * $mapred.local.dir/taskTracker/$user/jobcache/$jobid/jars/job.jar</li> <li>
 * $mapred.local.dir/taskTracker/$user/jobcache/$jobid/job.xml</li> <li>
 * $mapred.local.dir/taskTracker/$user/jobcache/$jobid/jobToken</li> <li>
 * $mapred.local.dir/taskTracker/$user/distcache</li>
 */
public class JobLocalizer
{
	...
	private static final FsPermission urwx = FsPermission
			.createImmutable((short) 0700);
	private static final FsPermission urwx_gx = FsPermission
			.createImmutable((short) 0710);
	 ...
	 * localization code, only if the job is not already localized. <br>
	 ...
	 * {@link TaskController#initializeJob} is run at a later time to set proper
	 * private permissions on the job directories. <br>
	 */
	public void createJobDirs() throws IOException
	{
		...
		for (Path localDir : localDirs)
		{
			...
			if (lfs.exists(fullJobDir))
			{
				...
				// Sometimes
				// copying job.xml to the local disk succeeds but copying
				// job.jar might
				// throw out an exception. We should clean up and then try
				// again.
	...
	/**
	 * Create job log directory and set appropriate permissions for the
	 * directory.
	 */
	public void initializeJobLogDir() throws IOException
	{
		Path jobUserLogDir = new Path(TaskLog.getJobDir(jobid).toURI()
				.toString());
	...
	private void localizeJobJarFile(JobConf localJobConf) throws IOException
	{
		...
		if (jarFile != null)
		{
			...
			// Here we check for five times the size of jarFileSize to
			// accommodate for
			// unjarring the jar file in the jars directory
			Path localJarFile = lDirAlloc.getLocalPathForWrite(JARDST,
					5 * jarFileSize, ttConf);
			...
			RunJar.unJar(new File(localJarFile.toString()), new File(
					localJarFile.getParent().toString()), localJobConf
					.getJarUnpackPattern());
			try
			{
				FileUtil.chmod(localJarFile.getParent().toString(), "ugo+rx",
						true);
			}
			catch (InterruptedException ie)
			{
				// This exception is never actually thrown, but the signature
				// says
	...
	/**
	 * The permissions to use for the private distributed cache objects. It is
	 * already protected by the user directory, so keep the group and other the
	 * same so that LocalFileSystem will use the java File methods to set
	 * permission.
	 */
	private static final FsPermission privateCachePerms = FsPermission
	 ...
	 * @param sources
	 *            the list of objects to download from
	 * @param dests
	 *            the list of paths to download them to
	 * @param times
	 *            the desired modification times
	 * @param isPublic
	 *            are the objects in the public cache?
	 * @param isArchive
	 *            are these archive files?
	 * @throws IOException
	...
	private static long[] downloadPrivateCacheObjects(Configuration conf,
			URI[] sources, Path[] dests, long[] times, boolean[] isPublic,
			boolean isArchive) throws IOException
	{
		if (null == sources && null == dests && null == times
				&& null == isPublic)
		{
			return null;
		}
		if (sources.length != dests.length || sources.length != times.length
				|| sources.length != isPublic.length)
		{
			throw new IOException(
					"Distributed cache entry arrays have different "
							+ "lengths: " + sources.length + ", "
							+ dests.length + ", " + times.length + ", "
							+ isPublic.length);
		}
		...
		for (int i = 0; i < sources.length; i++)
		{
			...
			if (!isPublic[i])
			{
				result[i] = TrackerDistributedCacheManager.downloadCacheObject(
						conf, sources[i], dests[i], times[i], isArchive,
						privateCachePerms);
			}
		}
		return result;
	...
	public static long[] downloadPrivateCache(Configuration conf)
			throws IOException
	{
		downloadPrivateCacheObjects(conf, DistributedCache.getCacheFiles(conf),
				DistributedCache.getLocalCacheFiles(conf),
				DistributedCache.getFileTimestamps(conf),
				TrackerDistributedCacheManager.getFileVisibilities(conf), false);
		return downloadPrivateCacheObjects(conf,
				DistributedCache.getCacheArchives(conf),
				DistributedCache.getLocalCacheArchives(conf),
				DistributedCache.getArchiveTimestamps(conf),
				TrackerDistributedCacheManager.getArchiveVisibilities(conf),
				true);
	}
			...
			throws IOException, InterruptedException
	{
		localizeJobFiles(jobid, jConf,
				lDirAlloc.getLocalPathForWrite(JOBCONF, ttConf),
	...
	public void localizeJobFiles(final JobID jobid, JobConf jConf,
			...
			final TaskUmbilicalProtocol taskTracker) throws IOException,
			InterruptedException
	{
		...
		if (sizes != null)
		{
			...
			// calls the localizeJobFiles method in the context of the
			// TaskTracker
			// process. The JVM authorization check would fail without this
			// doAs. In the LinuxTC case, this doesn't harm.
			UserGroupInformation ugi = UserGroupInformation
					.createRemoteUser(jobid.toString());
			ugi.doAs(new PrivilegedExceptionAction<Object>()
			{
				public Object run() throws IOException
				{
					taskTracker
							.updatePrivateDistributedCacheSizes(jobid, sizes);
					return null;
				}
			});
		...
		writeJobACLs(jConf, new Path(TaskLog.getJobDir(jobid).toURI()
				.toString()));
	 ...
	 * file. queue name is the queue to which the job was submitted to.
	 * queue-admins-acl is the queue admins ACL of the queue to which this job
	 * was submitted to.
	 * 
	 * @param conf
	 *            job configuration
			...
			InterruptedException
	{
		...
		final InetSocketAddress ttAddr = new InetSocketAddress(argv[2],
				Integer.parseInt(argv[3]));
		final String uid = UserGroupInformation.getCurrentUser()
				.getShortUserName();
		if (!user.equals(uid))
		{
		...
		final Credentials creds = TokenCache.loadTokens(jobTokenFile.toUri()
				.toString(), conf);
		...
		UserGroupInformation ugiJob = UserGroupInformation
				.createRemoteUser(jobid);
		...
		final TaskUmbilicalProtocol taskTracker = ugiJob
				.doAs(new PrivilegedExceptionAction<TaskUmbilicalProtocol>()
				{
					...
					{
						TaskUmbilicalProtocol taskTracker = (TaskUmbilicalProtocol) RPC
								.getProxy(TaskUmbilicalProtocol.class,
