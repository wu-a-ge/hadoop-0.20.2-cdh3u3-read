...
/**
 * This class groups the fundamental classes associated with reading/writing
 * splits. The split information is divided into two parts based on the consumer
 * of the information. The two parts are the split meta information, and the raw
 * split information. The first part is consumed by the JobTracker to create the
 * tasks' locality data structures. The second part is used by the maps at
 * runtime to know what to do! These pieces of information are written to two
 * separate files. The metainformation file is slurped by the JobTracker during
 * job initialization. A map task gets the meta information during the launch
 * and it reads the raw split bytes directly from the file.
 */
public class JobSplit
{
	...
	/**
	 * This represents the meta information about the task split. The main
	 * fields are - start offset in actual split - data length that will be
	 * processed in this split - hosts on which this split is local
	 */
	...
	public static class TaskSplitMetaInfo
	{
		...
		public TaskSplitMetaInfo(String[] locations, long startOffset,
				long inputDataLength)
		{
			this(new TaskSplitIndex("", startOffset), locations,
					inputDataLength);
		}
	...
	/**
	 * This represents the meta information about the task split that the task
	 * gets
	 */
