...
/**
 * A (internal) utility that reads the split meta info and creates split meta
 * info objects
 */
...
public class SplitMetaInfoReader
{
			...
			throws IOException
	{
		long maxMetaInfoSize = conf.getLong(
				"mapreduce.jobtracker.split.metainfo.maxsize", 10000000L);
		...
		final int maxLocations = conf.getInt(
				JobSplitWriter.MAX_SPLIT_LOCATIONS, Integer.MAX_VALUE);
		for (int i = 0; i < numSplits; i++)
		{
			...
			if (numLocations > maxLocations)
			{
				throw new IOException(
						"Max block location exceeded for split: #" + i
								+ " splitsize: " + numLocations + " maxsize: "
								+ maxLocations);
			}
