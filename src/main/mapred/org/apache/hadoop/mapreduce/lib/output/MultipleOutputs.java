...
public class MultipleOutputs<KEYOUT, VALUEOUT>
{
	 ...
	 */
	private static final String COUNTERS_GROUP = MultipleOutputs.class
			.getName();
			...
			JobContext job, String namedOutput)
	{
		return (Class<? extends OutputFormat<?, ?>>) job.getConfiguration()
				.getClass(MO_PREFIX + namedOutput + FORMAT, null,
						OutputFormat.class);
	}
			...
			TaskInputOutputContext<?, ?, KEYOUT, VALUEOUT> context)
	{
		...
		namedOutputs = Collections.unmodifiableSet(new HashSet<String>(
				MultipleOutputs.getNamedOutputsList(context)));
			...
			throws IOException, InterruptedException
	{
		...
		if (writer == null)
		{
			...
			if (countersEnabled)
			{
				writer = new RecordWriterWithCounter(writer, baseFileName,
						context);
			}
	...
	private TaskAttemptContext getContext(String nameOutput) throws IOException
	{
		...
		taskContext = new TaskAttemptContext(job.getConfiguration(),
