		...
		FileInputFormat<K, V>
{
	 ...
	 */
	private void getMoreSplits(Configuration conf, Path[] paths, long maxSize,
			...
			throws IOException
	{
		...
		for (int i = 0; i < paths.length; i++)
		{
			files[i] = new OneFileInfo(paths[i], conf, rackToBlocks,
					blockToNodes, nodeToBlocks, rackToNodes);
				...
				for (OneBlockInfo oneblock : blocks)
				{
					if (blockToNodes.containsKey(oneblock))
					{
						...
						if (maxSize != 0 && curSplitSize >= maxSize)
						{
							...
							// array
							addCreatedSplit(splits, getHosts(racks),
									validBlocks);
							createdSplit = true;
							break;
						}
	...
	private static class OneFileInfo
	{
		...
		OneFileInfo(Path path, Configuration conf,
				...
				HashMap<String, Set<String>> rackToNodes) throws IOException
		{
			this.fileSize = 0;
			...
			else
			{
				...
				for (int i = 0; i < locations.length; i++)
				{
					...
					for (int j = 0; j < oneblock.racks.length; j++)
					{
						...
						addHostToRack(rackToNodes, oneblock.racks[j],
								oneblock.hosts[j]);
					}
