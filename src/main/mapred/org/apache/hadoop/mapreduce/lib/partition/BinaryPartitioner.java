...
/**
 * <p>
 * Partition {@link BinaryComparable} keys using a configurable part of the
 * bytes array returned by {@link BinaryComparable#getBytes()}.
 * </p>
 * 
 * <p>
 * The subarray to be used for the partitioning can be defined by means of the
 * following properties:
 * <ul>
 * <li>
 * <i>mapred.binary.partitioner.left.offset</i>: left offset in array (0 by
 * default)</li>
 * <li>
 * <i>mapred.binary.partitioner.right.offset</i>: right offset in array (-1 by
 * default)</li>
 * </ul>
 * Like in Python, both negative and positive offsets are allowed, but the
 * meaning is slightly different. In case of an array of length 5, for instance,
 ...
 * array; the second row gives the corresponding negative offsets. Contrary to
 * Python, the specified subarray has byte <code>i</code> and <code>j</code> as
 * first and last element, repectively, when <code>i</code> and <code>j</code>
 * are the left and right offset.
 * 
 * <p>
 * For Hadoop programs written in Java, it is advisable to use one of the
 * following static convenience methods for setting the offsets:
 * <ul>
		...
		implements Configurable
{
	...
	private static final String LEFT_OFFSET_PROPERTY_NAME = "mapred.binary.partitioner.left.offset";
	private static final String RIGHT_OFFSET_PROPERTY_NAME = "mapred.binary.partitioner.right.offset";
	...
	public int getPartition(BinaryComparable key, V value, int numPartitions)
	{
		...
		int hash = WritableComparator.hashBytes(key.getBytes(), leftIndex,
