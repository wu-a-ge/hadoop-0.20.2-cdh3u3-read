...
public class FileOutputCommitter extends OutputCommitter
{
			...
			throws IOException
	{
		if (outputPath != null)
		{
			...
			outputFileSystem = outputPath.getFileSystem(context
					.getConfiguration());
			workPath = new Path(outputPath, (FileOutputCommitter.TEMP_DIR_NAME
					+ Path.SEPARATOR + "_" + context.getTaskAttemptID()
					.toString())).makeQualified(outputFileSystem);
		}
	}
	...
	private void markOutputDirSuccessful(JobContext context) throws IOException
	{
		if (outputPath != null)
		{
			FileSystem fileSys = outputPath.getFileSystem(context
					.getConfiguration());
	 ...
	 * @throws IOException
	 */
	private void moveTaskOutputs(TaskAttemptContext context, FileSystem fs,
