...
public class Localizer
{
	...
	public void initializeUserDirs(String user) throws IOException
	{
		...
		synchronized (localizedUser)
		{
			...
			if (localizedUser.get())
			{
				...
				LOG.info("User-directories for the user "
						+ user
			...
			for (String localDir : localDirs)
			{
				...
				if (fs.exists(userDir) || fs.mkdirs(userDir))
				{
					...
					Path jobCacheDir = new Path(localDir,
							TaskTracker.getJobCacheSubdir(user));
					if (fs.exists(jobCacheDir) || fs.mkdirs(jobCacheDir))
					{
						// Set permissions on the jobcache-directory
						fs.setPermission(jobCacheDir, new FsPermission(
								(short) 0700));
					...
					// Set up the cache directory used for distributed cache
					// files
					Path distributedCacheDir = new Path(localDir,
							TaskTracker.getPrivateDistributedCacheDir(user));
					if (fs.exists(distributedCacheDir)
							|| fs.mkdirs(distributedCacheDir))
					{
						// Set permissions on the distcache-directory
						fs.setPermission(distributedCacheDir, new FsPermission(
								(short) 0700));
			...
			String attemptId) throws IOException
	{
		...
		String attemptDirPath = TaskTracker.getLocalTaskDir(user, jobId,
				attemptId);
		...
		{
			throw new IOException(
					"Not able to initialize attempt directories "
	...
	{
		Path jobUserLogDir = new Path(TaskLog.getJobDir(jobId)
				.getCanonicalPath());
