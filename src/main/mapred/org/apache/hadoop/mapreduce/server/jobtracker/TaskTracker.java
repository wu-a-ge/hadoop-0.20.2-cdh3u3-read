...
/**
 * The representation of a single <code>TaskTracker</code> as seen by the
 * {@link JobTracker}.
 */
public class TaskTracker
{
	 ...
	 * 
	 * @param trackerName
	 *            Unique identifier for the <code>TaskTracker</code>
	 */
	 ...
	 * 
	 * @param status
	 *            the current <code>TaskTrackerStatus</code> of the
	 *            <code>TaskTracker</code>
	 */
	 ...
	 * 
	 * @param taskType
	 *            the {@link TaskType} to check for number of available slots
	...
	public int getAvailableSlots(TaskType taskType)
	{
		...
		if (taskType == TaskType.MAP)
		{
			...
			{
				LOG.debug(trackerName + " getAvailSlots:" + " max(m)="
						+ status.getMaxMapSlots() + " occupied(m)="
		...
		else
		{
			if (LOG.isDebugEnabled())
			{
				LOG.debug(trackerName + " getAvailSlots:" + " max(r)="
						+ status.getMaxReduceSlots() + " occupied(r)="
	 ...
	 * 
	 * @param taskType
	 *            {@link TaskType} of the task
	 * @return the task for which the fallow slot(s) are held, <code>null</code>
	 *         if there are no fallow slots
	 */
	public JobInProgress getJobForFallowSlot(TaskType taskType)
	{
		return (taskType == TaskType.MAP) ? jobForFallowMapSlot
	 ...
	 *            {@link TaskType} of the task
	 * @param job
	 *            the job for which slots on this <code>TaskTracker</code> are
	 *            to be reserved
	 * @param numSlots
	...
	public void reserveSlots(TaskType taskType, JobInProgress job, int numSlots)
	{
		...
		if (taskType == TaskType.MAP)
		{
					...
					&& !jobForFallowMapSlot.getJobID().equals(jobId))
			{
				throw new RuntimeException(trackerName + " already has "
						+ "slots reserved for " + jobForFallowMapSlot
						+ "; being" + " asked to reserve " + numSlots + " for "
						+ jobId);
			}
		...
		else if (taskType == TaskType.REDUCE)
		{
			...
			{
				throw new RuntimeException(trackerName + " already has "
						+ "slots reserved for " + jobForFallowReduceSlot
						+ "; being" + " asked to reserve " + numSlots + " for "
						+ jobId);
			}
	 ...
	 *            {@link TaskType} of the task
	 * @param job
	 *            job whose slots are being un-reserved
	 */
	public void unreserveSlots(TaskType taskType, JobInProgress job)
	{
		...
		if (taskType == TaskType.MAP)
		{
					...
					|| !jobForFallowMapSlot.getJobID().equals(jobId))
			{
				throw new RuntimeException(trackerName + " already has "
						+ "slots reserved for " + jobForFallowMapSlot
						+ "; being" + " asked to un-reserve for " + jobId);
			}
		...
		LOG.info(trackerName + ": Unreserved " + taskType + " slots for "
				+ jobId);
	}

	/**
	 * Cleanup when the {@link TaskTracker} is declared as 'lost/blacklisted' by
	 * the JobTracker.
	 * 
	 * The method assumes that the lock on the {@link JobTracker} is obtained by
	 * the caller.
	 */
