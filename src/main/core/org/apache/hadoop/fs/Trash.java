...
public class Trash extends Configured
{
	...
	public Trash(FileSystem fs, Configuration conf) throws IOException
	{
		...
		this.interval = conf.getLong("fs.trash.interval", 60)
				* MSECS_PER_MINUTE;
	}
	...
	public boolean moveToTrash(Path path) throws IOException
	{
		...
		for (int i = 0; i < 2; i++)
		{
			try
			{
				...
				{ // create current
					LOG.warn("Can't create(mkdir) trash directory: "
							+ baseTrashPath);
					return false;
				}
			}
	...
	public void checkpoint() throws IOException
	{
		...
		{
			LOG.info("Created trash checkpoint: "
	...
	public void expunge() throws IOException
	{
		...
		for (int i = 0; i < dirs.length; i++)
		{
			...
			if ((now - interval) > time)
			{
				...
				{
					LOG.warn("Couldn't delete checkpoint: " + dir
							+ " Ignoring.");
				}
			}
		}
			...
			while (true)
			{
				...
				try
				{
					...
					if (now >= end)
					{
						...
						{
							LOG.warn("Trash can't list homes: " + e
									+ " Sleeping.");
							continue;
						}
