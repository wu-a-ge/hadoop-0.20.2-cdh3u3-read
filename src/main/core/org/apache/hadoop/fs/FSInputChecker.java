...
abstract public class FSInputChecker extends FSInputStream
{
	 ...
	 */
	protected FSInputChecker(Path file, int numOfRetries,
			boolean verifyChecksum, Checksum sum, int chunkSize,
			int checksumSize)
	{
			...
			throws IOException
	{
		...
		do
		{
			...
			{
				LOG.info(
						"Found checksum error: b["
								+ off
								+ ", "
								+ (off + read)
								+ "]="
								+ StringUtils.byteToHexString(b, off, off
										+ read), ce);
				if (retriesLeft == 0)
				{
					throw ce;
	 ...
	 */
	protected static int readFully(InputStream stm, byte[] buf, int offset,
	 ...
	 *            checksum size
	 */
	final protected synchronized void set(boolean verifyChecksum, Checksum sum,
			int maxChunkSize, int checksumSize)
	{
