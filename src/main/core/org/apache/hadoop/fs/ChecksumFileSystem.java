...
public abstract class ChecksumFileSystem extends FilterFileSystem
{
	...
	private static class ChecksumFSInputChecker extends FSInputChecker
	{
				...
				byte[] checksum) throws IOException
		{
			...
			if (eof && nread > 0)
			{
				throw new ChecksumException("Checksum error: " + file + " at "
						+ pos, pos);
			}
			return nread;
		}
	...
	{
		return new FSDataInputStream(new ChecksumFSInputChecker(this, f,
				bufferSize));
	}
	...
	private static class ChecksumFSOutputSummer extends FSOutputSummer
	{
		...
		public ChecksumFSOutputSummer(ChecksumFileSystem fs, Path file,
				boolean overwrite, short replication, long blockSize,
				Configuration conf) throws IOException
		{
			this(fs, file, overwrite, conf.getInt("io.file.buffer.size", 4096),
				...
				long blockSize, Progressable progress) throws IOException
		{
			...
			this.datas = fs.getRawFileSystem().create(file, overwrite,
					bufferSize, replication, blockSize, progress);
			int sumBufferSize = fs.getSumBufferSize(bytesPerSum, bufferSize);
			this.sums = fs.getRawFileSystem().create(fs.getChecksumFile(file),
					true, sumBufferSize, replication, blockSize);
			...
			throws IOException
	{
		...
		final FSDataOutputStream out = new FSDataOutputStream(
				new ChecksumFSOutputSummer(this, f, overwrite, bufferSize,
			...
			throws IOException
	{
		...
		else
		{
			...
			{
				copyToLocalFile(srcFile.getPath(), new Path(dst, srcFile
						.getPath().getName()), copyCrc);
			}
		}
	}
