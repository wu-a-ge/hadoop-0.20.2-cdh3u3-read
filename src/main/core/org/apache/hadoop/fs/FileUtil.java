...
public class FileUtil
{
		...
		else
		{
			...
			if (!sdst.isDir())
				throw new IOException(
						"copying multiple files, but last argument `" + dst
								+ "' is not a directory");
		}
	...
	public static boolean copy(File src, FileSystem dstFS, Path dst,
			...
			boolean deleteSource, Configuration conf) throws IOException
	{
		if (srcFS.getFileStatus(src).isDir())
		{
			...
			for (int i = 0; i < contents.length; i++)
			{
				copy(srcFS, contents[i].getPath(), new File(dst, contents[i]
						.getPath().getName()), deleteSource, conf);
			}
		}
		else if (srcFS.isFile(src))
		{
	...
	public static class HardLink
	{
		...
		static
		{
			...
			{
			case OS_TYPE_WINXP:
				hardLinkCommand = new String[] {
						"fsutil", "hardlink", "create", null, null
				};
				getLinkCountCommand = new String[] {
						"stat", "-c%h"
				};
				break;
		 ...
		 * Creates a hardlink
		 */
		public static void createHardLink(File target, File linkName)
				throws IOException
		{
		...
		public static int getLinkCount(File fileName) throws IOException
		{
			...
			{
				throw new IOException(StringUtils.stringifyException(e)
						+ inpMsg + errMsg + " on file:" + fileName);
			}
			catch (InterruptedException e)
			{
				throw new IOException(StringUtils.stringifyException(e)
	...
	public static final File createLocalTempFile(final File basefile,
			final String prefix, final boolean isDeleteOnExit)
			throws IOException
	{
	...
	public static File[] listFiles(File dir) throws IOException
	{
		...
		{
			throw new IOException(
					"Invalid directory or I/O error occurred for dir: "
							+ dir.toString());
		}
		return files;
	}
