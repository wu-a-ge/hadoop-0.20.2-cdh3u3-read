...
public class SequenceFile
{
	...
	static public CompressionType getCompressionType(Configuration job)
	{
		...
		return name == null ? CompressionType.RECORD : CompressionType
				.valueOf(name);
	}
	 ...
	 */
	public static Writer createWriter(FileSystem fs, Configuration conf,
			Path name, Class keyClass, Class valClass,
			CompressionType compressionType) throws IOException
	{
		return createWriter(fs, conf, name, keyClass, valClass, fs.getConf()
	...
	public static Writer createWriter(FileSystem fs, Configuration conf,
			Path name, Class keyClass, Class valClass,
			CompressionType compressionType, Progressable progress)
			throws IOException
	{
		return createWriter(fs, conf, name, keyClass, valClass, fs.getConf()
				.getInt("io.file.buffer.size", 4096),
	...
	public static Writer createWriter(FileSystem fs, Configuration conf,
			...
			short replication, long blockSize, CompressionType compressionType,
			...
			throws IOException
	{
				...
				&& !ZlibFactory.isNativeZlibLoaded(conf))
		{
			throw new IllegalArgumentException(
					"SequenceFile doesn't work with "
		...
		{
			writer = new Writer(fs, conf, name, keyClass, valClass, bufferSize,
					replication, blockSize, progress, metadata);
		}
		else if (compressionType == CompressionType.RECORD)
		{
			writer = new RecordCompressWriter(fs, conf, name, keyClass,
					valClass, bufferSize, replication, blockSize, codec,
					progress, metadata);
		}
		else if (compressionType == CompressionType.BLOCK)
		{
			writer = new BlockCompressWriter(fs, conf, name, keyClass,
					valClass, bufferSize, replication, blockSize, codec,
					progress, metadata);
		}
			...
			Metadata metadata) throws IOException
	{
		...
		{
		case NONE:
			return new Writer(conf, fsos, keyClass, valClass, metadata)
					.ownStream();
		case RECORD:
			return new RecordCompressWriter(conf, fsos, keyClass, valClass,
					codec, metadata).ownStream();
		case BLOCK:
			return new BlockCompressWriter(conf, fsos, keyClass, valClass,
					codec, metadata).ownStream();
		default:
			return null;
		}
	 ...
	 * @throws IOException
	 */
	private static Writer createWriter(Configuration conf,
			FSDataOutputStream out, Class keyClass, Class valClass,
			boolean compress, boolean blockCompress, CompressionCodec codec,
			Metadata metadata) throws IOException
	{
		...
		{
			writer = new RecordCompressWriter(conf, out, keyClass, valClass,
					codec, metadata);
		}
		else
		{
			writer = new BlockCompressWriter(conf, out, keyClass, valClass,
					codec, metadata);
		}
	 ...
	 * @throws IOException
	 */
	private static Writer createWriter(FileSystem fs, Configuration conf,
			Path file, Class keyClass, Class valClass, boolean compress,
			boolean blockCompress, CompressionCodec codec,
			Progressable progress, Metadata metadata) throws IOException
	{
		if (codec != null && (codec instanceof GzipCodec)
				&& !NativeCodeLoader.isNativeCodeLoaded()
		...
		if (!compress)
		{
			writer = new Writer(fs, conf, file, keyClass, valClass, progress,
					metadata);
		}
		else if (compress && !blockCompress)
		{
			writer = new RecordCompressWriter(fs, conf, file, keyClass,
					valClass, codec, progress, metadata);
		}
		else
		{
			writer = new BlockCompressWriter(fs, conf, file, keyClass,
					valClass, codec, progress, metadata);
		}
	 ...
	 * @throws IOException
	 */
	public static Writer createWriter(Configuration conf,
			FSDataOutputStream out, Class keyClass, Class valClass,
			CompressionType compressionType, CompressionCodec codec,
			Metadata metadata) throws IOException
	{
			...
			throws IOException
	{
		Writer writer = createWriter(conf, out, keyClass, valClass,
				compressionType, codec, new Metadata());
		return writer;
	}
	...
	public static class Writer implements java.io.Closeable
	{
		...
		public Writer(FileSystem fs, Configuration conf, Path name,
				...
				Metadata metadata) throws IOException
		{
			this(fs, conf, name, keyClass, valClass, fs.getConf().getInt(
					"io.file.buffer.size", 4096), fs.getDefaultReplication(),
				...
				Metadata metadata) throws IOException
		{
			init(name, conf, fs.create(name, true, bufferSize, replication,
					blockSize, progress), keyClass, valClass, false, null,
					metadata);
				...
				CompressionCodec codec, Metadata metadata) throws IOException
		{
			...
			if (this.codec != null)
			{
				...
				this.deflateFilter = this.codec.createOutputStream(buffer,
						compressor);
				this.deflateOut = new DataOutputStream(
	...
	static class RecordCompressWriter extends Writer
	{
		...
		/** Create the named file. */
		public RecordCompressWriter(FileSystem fs, Configuration conf,
				Path name, Class keyClass, Class valClass,
				CompressionCodec codec) throws IOException
		{
			this(conf, fs.create(name), keyClass, valClass, codec,
					new Metadata());
		}
				...
				throws IOException
		{
			this(fs, conf, name, keyClass, valClass, fs.getConf().getInt(
					"io.file.buffer.size", 4096), fs.getDefaultReplication(),
					fs.getDefaultBlockSize(), codec, progress, metadata);
		}

		/** Create the named file with write-progress reporter. */
		public RecordCompressWriter(FileSystem fs, Configuration conf,
				Path name, Class keyClass, Class valClass, int bufferSize,
				short replication, long blockSize, CompressionCodec codec,
				Progressable progress, Metadata metadata) throws IOException
		{
			super.init(name, conf, fs.create(name, true, bufferSize,
					replication, blockSize, progress), keyClass, valClass,
					true, codec, metadata);
		...
		public RecordCompressWriter(FileSystem fs, Configuration conf,
				...
				throws IOException
		{
			this(fs, conf, name, keyClass, valClass, codec, progress,
					new Metadata());
		}
	...
	static class BlockCompressWriter extends Writer
	{
		...
		/** Create the named file. */
		public BlockCompressWriter(FileSystem fs, Configuration conf,
				Path name, Class keyClass, Class valClass,
				CompressionCodec codec) throws IOException
		{
			this(fs, conf, name, keyClass, valClass, fs.getConf().getInt(
					"io.file.buffer.size", 4096), fs.getDefaultReplication(),
					fs.getDefaultBlockSize(), codec, null, new Metadata());
		}
		...
		private synchronized void writeBuffer(
				DataOutputBuffer uncompressedDataBuffer) throws IOException
		{
			deflateFilter.resetState();
			buffer.reset();
		...
		{
			if (key.getClass() != keyClass)
				throw new IOException("wrong key class: " + key + " is not "
						+ keyClass);
			if (val.getClass() != valClass)
	...
	public static class Reader implements java.io.Closeable
	{
		...
		private void init(boolean tempReader) throws IOException
		{
			...
			if (decompress)
			{
				if (version >= CUSTOM_COMPRESS_VERSION)
				{
					...
					try
					{
						...
						this.codec = ReflectionUtils.newInstance(codecClass,
								conf);
					}
			...
			if (!tempReader)
			{
				...
				if (decompress)
				{
					...
					valInFilter = codec.createInputStream(valBuffer,
							valDecompressor);
				...
				if (blockCompressed)
				{
					...
					keyInFilter = codec.createInputStream(keyBuffer,
							keyDecompressor);
				...
				SerializationFactory serializationFactory = new SerializationFactory(
						conf);
				this.keyDeserializer = getDeserializer(serializationFactory,
						getKeyClass());
				if (!blockCompressed)
				{
				...
				this.valDeserializer = getDeserializer(serializationFactory,
						getValueClass());
			...
			if (!blockCompressed)
			{
				...
				if (valBuffer.getPosition() != keyLength)
					throw new IOException(key + " read "
							+ valBuffer.getPosition() + " bytes, should read "
							+ keyLength);
			}
			else
			{
				...
				throws IOException
		{
			...
			{
				LOG.warn("Bad checksum at " + getPosition()
						+ ". Skipping entries.");
				sync(getPosition()
	...
	public static class Sorter
	{
		...
		private class SortPass
		{
			...
			public int run(boolean deleteInput) throws IOException
			{
				...
				while (!atEof)
				{
					...
					rawKeys.reset();
					while (!atEof && bytesProcessed < memoryLimit
							&& count < recordLimit)
					{
					...
					flush(count, bytesProcessed, isCompressed,
							isBlockCompressed, codec, segments == 0 && atEof);
					segments++;
				}
				return segments;
			}
			...
			private void flush(int count, int bytesProcessed,
					boolean isCompressed, boolean isBlockCompressed,
					CompressionCodec codec, boolean done) throws IOException
			{
				...
				Writer writer = createWriter(conf, out, keyClass, valClass,
						isCompressed, isBlockCompressed, codec, new Metadata());
				...
				{ // write in sorted order
					int p = pointers[i];
					writer.appendRaw(rawBuffer, keyOffsets[p], keyLengths[p],
							rawValues[p]);
				}
				writer.close();
				...
				if (!done)
				{
					...
					WritableUtils.writeVLong(indexOut,
							(out.getPos() - segmentStart));
					indexOut.flush();
				}
			}
				...
				Path tmpDir) throws IOException
		{
			return merge(inNames, deleteInputs,
					(inNames.length < factor) ? inNames.length : factor, tmpDir);
		}
		 ...
		 */
		public RawKeyValueIterator merge(Path[] inNames, boolean deleteInputs,
				int factor, Path tmpDir) throws IOException
		{
				...
				Progressable prog) throws IOException
		{
			...
			Writer writer = createWriter(outputFile.getFileSystem(conf), conf,
					outputFile, keyClass, valClass, compress, blockCompress,
					codec, prog, new Metadata());
			return writer;
		}
				...
				throws IOException
		{
			while (records.next())
			{
				writer.appendRaw(records.getKey().getData(), 0, records
						.getKey().getLength(), records.getValue());
			}
			writer.sync();
		}
		...
		private int mergePass(Path tmpDir) throws IOException
		{
			...
			Writer writer = cloneFileAttributes(outFile.suffix(".0"), outFile,
					null);
		...
		private class MergeQueue extends PriorityQueue implements
				RawKeyValueIterator
		{
			private boolean compress;
			 ...
			 */
			public MergeQueue(List<SegmentDescriptor> segments, Path tmpDir,
					Progressable progress)
			{
			...
			protected boolean lessThan(Object a, Object b)
			{
				...
				return comparator.compare(msa.getKey().getData(), 0, msa
						.getKey().getLength(), msb.getKey().getData(), 0, msb
						.getKey().getLength()) < 0;
			}
			...
			private void adjustPriorityQueue(SegmentDescriptor ms)
					throws IOException
			{
			...
			public RawKeyValueIterator merge() throws IOException
			{
				...
				do
				{
					...
					else
					{
						...
						SegmentDescriptor tempSegment = new SegmentDescriptor(
								0, fs.getLength(outputFile), outputFile);
		...
		public class SegmentDescriptor implements Comparable
		{
			...
			public boolean nextRawKey() throws IOException
			{
				if (in == null)
				{
					...
					Reader reader = new Reader(fs, segmentPathName, bufferSize,
							segmentOffset, segmentLength, conf, false);
		...
		private class LinkedSegmentsDescriptor extends SegmentDescriptor
		{
			 ...
			 */
			public LinkedSegmentsDescriptor(long segmentOffset,
					long segmentLength, Path segmentPathName,
