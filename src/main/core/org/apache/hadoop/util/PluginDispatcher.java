...
/**
 * Provides convenience functions for dispatching calls through to plugins
 * registered with a class. Classes that wish to provide plugin interfaces
 * should use this class to load the plugin list from the Configuration and to
 * dispatch calls to the loaded instances.
 * 
 * Calls dispatched through this class are performed on a second thread so as to
 * not block execution of the plugged service.
 */
public class PluginDispatcher<T extends ServicePlugin>
{
	public static final Log LOG = LogFactory.getLog(PluginDispatcher.class
			.getName());
	 ...
	 * 
	 * @param conf
	 *            the Configuration from which to load
	 * @param key
	 *            the configuration key that lists class names to instantiate
	 * @param clazz
	 *            the class or interface from which plugins must extend
	 */
	 ...
	 * 
	 * @param callback
	 *            a function which will run once for each plugin, with that
	 *            plugin as the argument
	 */
		...
		executor.execute(new Runnable()
		{
			public void run()
			{
				for (T plugin : plugins)
				{
					...
					{
						LOG.warn("Uncaught exception dispatching to plugin "
								+ plugin, t);
					}
				}
			}
	...
	/**
	 * Dispatches the start(...) hook common to all ServicePlugins. This also
	 * automatically removes any plugin that throws an exception while
	 * attempting to start.
	 * 
	 * @param plugPoint
	 *            passed to ServicePlugin.start()
	 */
