...
public abstract class Server
{
	...
	public class Connection
	{
		...
		private final Call authFailedCall = new Call(
				AUTHORIZATION_FAILED_CALLID, null, this);
				...
				throws IOException
		{
			...
			{
				TokenIdentifier tokenId = SaslRpcServer.getIdentifier(
						authorizedId, secretManager);
				...
				InterruptedException
		{
			if (!saslContextEstablished)
			{
				...
				try
				{
					if (saslServer == null)
					{
						...
						case DIGEST:
							...
							saslServer = Sasl.createSaslServer(
									AuthMethod.DIGEST.getMechanismName(), null,
						...
						default:
							...
							if (LOG.isDebugEnabled())
								LOG.debug("Kerberos principal name is "
										+ fullName);
							final String names[] = SaslRpcServer
					...
					if (LOG.isDebugEnabled())
						LOG.debug("Have read input token of size "
								+ saslToken.length
				...
				catch (IOException e)
				{
					...
					doSaslReply(SaslStatus.ERROR, null, sendToClient.getClass()
							.getName(), sendToClient.getLocalizedMessage());
				...
				{
					if (LOG.isDebugEnabled())
						LOG.debug("Will send token of size "
								+ replyToken.length + " from saslServer.");
					doSaslReply(SaslStatus.SUCCESS, new BytesWritable(
							replyToken), null, null);
				}
				if (saslServer.isComplete())
				{
					...
					String qop = (String) saslServer
							.getNegotiatedProperty(Sasl.QOP);
				...
				if (!rpcHeaderRead)
				{
					...
					if (isSecurityEnabled && authMethod == AuthMethod.SIMPLE)
					{
						...
						setupResponse(authFailedResponse, authFailedCall,
								Status.FATAL, null, ae.getClass().getName(),
								ae.getMessage());
					...
					if (!isSecurityEnabled && authMethod != AuthMethod.SIMPLE)
					{
						doSaslReply(SaslStatus.SUCCESS, new IntWritable(
								SaslRpcServer.SWITCH_TO_SIMPLE_AUTH), null,
								null);
		...
		private void processHeader(byte[] buf) throws IOException
		{
			...
			else
			{
				...
				if ((protocolUser != null)
						&& (!protocolUser.getUserName().equals(
								user.getUserName())))
				{
		...
		private void processUnwrappedData(byte[] inBuf) throws IOException,
				InterruptedException
		{
			ReadableByteChannel ch = Channels
					.newChannel(new ByteArrayInputStream(inBuf));
			...
			while (true)
			{
				...
				if (unwrappedData == null)
				{
					...
					int unwrappedDataLength = unwrappedDataLengthBuffer
							.getInt();
				...
				InterruptedException
		{
			...
			try
			{
				param = ReflectionUtils.newInstance(paramClass, conf);// read
																		// param
			...
			catch (Throwable t)
			{
				...
				setupResponse(
						...
						null,
						t.getClass().getName(),
						"IPC server unable to read call parameters: "
								+ t.getMessage());
	...
	{
		this(bindAddress, port, paramClass, handlerCount, conf, serverName,
				null);
	}
			...
			throws IOException
	{
		...
		if (status == Status.SUCCESS)
		{
			...
			catch (Throwable t)
			{
				...
				setupResponse(response, call, Status.ERROR, null, t.getClass()
						.getName(), StringUtils.stringifyException(t));
				return;
			}
		}
		else
			...
			throws IOException
	{
		if (call.connection.useSasl)
		{
			...
			if (LOG.isDebugEnabled())
				LOG.debug("Adding saslServer wrapped token of size "
						+ token.length + " as call response.");
			response.reset();
