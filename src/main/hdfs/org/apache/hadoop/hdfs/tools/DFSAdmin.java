...
public class DFSAdmin extends FsShell
{
	...
	private static class ClearQuotaCommand extends DFSAdminCommand
	{
		...
		{
			dfs.setQuota(path, FSConstants.QUOTA_RESET,
	...
	private static class ClearSpaceQuotaCommand extends DFSAdminCommand
	{
		...
		public void run(Path path) throws IOException
		{
			dfs.setQuota(path, FSConstants.QUOTA_DONT_SET,
	...
	public void report() throws IOException
	{
		if (fs instanceof DistributedFileSystem)
		{
			...
			boolean mode = dfs
					.setSafeMode(FSConstants.SafeModeAction.SAFEMODE_GET);
			 ...
			 * iteration of an internal list. Should be updated in a few seconds
			 * to minutes. Use "-metaSave" to list of all such blocks and
			 * accurate counts.
			 */
			...
			System.out
					.println("Missing blocks: " + dfs.getMissingBlocksCount());
			...
			System.out.println("Datanodes available: " + live.length + " ("
					+ (live.length + dead.length) + " total, " + dead.length
					+ " dead)\n");
	...
	/**
	 * Command to ask the namenode to save the namespace. Usage: java DFSAdmin
	 * -saveNamespace
	 * 
	 * @exception IOException
	...
	public int upgradeProgress(String[] argv, int idx) throws IOException
	{
		...
		String statusText = (status == null ? "There are no upgrades in progress."
	...
	public int refreshServiceAcl() throws IOException
	{
		...
		// Create the client
		RefreshAuthorizationPolicyProtocol refreshProtocol = (RefreshAuthorizationPolicyProtocol) RPC
				.getProxy(RefreshAuthorizationPolicyProtocol.class,
						RefreshAuthorizationPolicyProtocol.versionID, NameNode
								.getAddress(conf), getUGI(), conf,
	...
	public int refreshUserToGroupsMappings() throws IOException
	{
		...
		// Create the client
		RefreshUserMappingsProtocol refreshProtocol = (RefreshUserMappingsProtocol) RPC
				.getProxy(RefreshUserMappingsProtocol.class,
						RefreshUserMappingsProtocol.versionID, NameNode
								.getAddress(conf), getUGI(), conf, NetUtils
	...
	private static void printUsage(String cmd)
	{
		...
		{
			System.err
					.println("Usage: java DFSAdmin" + " [-metasave filename]");
		}
		else if (SetQuotaCommand.matches(cmd))
		{
			System.err.println("Usage: java DFSAdmin" + " ["
		...
		{
			System.err
					.println("Usage: java DFSAdmin" + " [-refreshServiceAcl]");
		}
		else if ("-refreshUserToGroupsMappings".equals(cmd))
		{
			System.err.println("Usage: java DFSAdmin"
		...
		else
		{
			...
			System.err
					.println("           [-refreshSuperUserGroupsConfiguration]");
