...
class BlocksMap
{
			...
			LightWeightGSet.LinkedElement
	{
		...
		BlockInfo getPrevious(int index)
		{
			...
			assert info == null
					|| BlockInfo.class.getName().equals(
							info.getClass().getName()) : "BlockInfo is expected at "
					+ index * 3;
			return info;
		}
		...
		{
			int count = 0;
			for (BlockInfo cur = this; cur != null; cur = cur.getNext(cur
					.findDatanode(dn)))
				count++;
			return count;
		}
		...
		boolean listIsConsistent(DatanodeDescriptor dn)
		{
			...
			while (cur != null)
			{
				...
				if (next != null)
				{
					...
					{
						System.out
								.println("Inconsistent list: cur->next->prev != cur");
						return false;
					}
				}
				cur = next;
