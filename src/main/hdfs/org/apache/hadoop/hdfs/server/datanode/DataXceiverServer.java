...
class DataXceiverServer implements Runnable, FSConstants
{
	...
	Map<Socket, Socket> childSockets = Collections
			.synchronizedMap(new HashMap<Socket, Socket>());
	...
	DataXceiverServer(ServerSocket ss, Configuration conf, DataNode datanode)
	{
		...
		this.balanceThrottler = new BlockBalanceThrottler(conf.getLong(
				"dfs.balance.bandwidthPerSec", 1024L * 1024));
	}
   ...
   */
	public void run()
	{
		while (datanode.shouldRun)
		{
			...
			{
				LOG.warn(datanode.dnRegistration
						+ ":DataXceiverServer: IOException due to:"
						+ StringUtils.stringifyException(ie));
			}
			catch (Throwable te)
			{
				LOG.error(datanode.dnRegistration
		...
		{
			LOG.warn(datanode.dnRegistration
					+ ":DataXceiverServer: Close exception due to: "
					+ StringUtils.stringifyException(ie));
		}
	...
	void kill()
	{
		...
		{
			for (Iterator<Socket> it = childSockets.values().iterator(); it
					.hasNext();)
			{
