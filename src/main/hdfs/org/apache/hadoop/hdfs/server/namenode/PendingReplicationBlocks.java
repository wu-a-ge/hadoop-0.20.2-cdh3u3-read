...
class PendingReplicationBlocks
{
	...
	void add(Block block, int numReplicas)
	{
		synchronized (pendingReplications)
		{
			...
			if (found == null)
			{
				pendingReplications.put(block,
						new PendingBlockInfo(numReplicas));
			}
			else
			{
	...
	Block[] getTimedOutBlocks()
	{
		synchronized (timedOutItems)
		{
			...
			Block[] blockList = timedOutItems.toArray(new Block[timedOutItems
					.size()]);
			timedOutItems.clear();
			return blockList;
		}
		...
		void pendingReplicationCheck()
		{
			synchronized (pendingReplications)
			{
				...
				while (iter.hasNext())
				{
					...
					PendingBlockInfo pendingBlock = (PendingBlockInfo) entry
							.getValue();
