...
public class NameNodeResourceChecker
{
	private static final Log LOG = LogFactory
			.getLog(NameNodeResourceChecker.class.getName());
	...
	/**
	 * Create a NameNodeResourceChecker, which will check the name dirs and
	 * edits dirs set in <code>conf</code>.
	 * 
	 * @param conf
	 * @throws IOException
	 */
	 ...
	 *            The directories whose volumes will be checked for available
	 *            space.
	 * @throws IOException
	 */
	...
	Collection<String> getVolumesLowOnSpace() throws IOException
	{
		...
		{
			LOG.debug("Going to check the following volumes disk space: "
					+ volumes);
		}
		...
		for (DF volume : volumes.values())
		{
			...
			if (LOG.isDebugEnabled())
			{
				LOG.debug("Space available on volume '" + fileSystem + "' is "
						+ availableSpace);
			}
