...
class DataXceiver extends Thread implements Runnable, FSConstants
{
	...
	public void run()
	{
		...
		try
		{
			...
			do
			{
				...
				catch (IOException err)
				{
					...
					if (opsProcessed > 0
							&& (err instanceof EOFException
									|| err instanceof ClosedChannelException || err
									.getMessage().contains(
											"Connection reset by peer")))
					{
						if (LOG.isDebugEnabled())
						{
							LOG.debug("Cached " + s.toString()
									+ " closing after " + opsProcessed + " ops");
						}
					}
					else
					{
				...
				// Indentation is left alone here so that patches merge easier
				// from 0.20.20x
				...
				case DataTransferProtocol.OP_READ_BLOCK:
					readBlock(in);
					datanode.myMetrics.readBlockOp.inc(DataNode.now()
							- startTime);
					if (local)
				...
				case DataTransferProtocol.OP_WRITE_BLOCK:
					writeBlock(in);
					datanode.myMetrics.writeBlockOp.inc(DataNode.now()
							- startTime);
					if (local)
				...
				case DataTransferProtocol.OP_COPY_BLOCK:
					...
					copyBlock(in);
					datanode.myMetrics.copyBlockOp.inc(DataNode.now()
							- startTime);
					break;
				case DataTransferProtocol.OP_BLOCK_CHECKSUM: // get the checksum
																// of a block
					getBlockChecksum(in);
		...
		{
			LOG.debug(datanode.dnRegistration
	...
	private void readBlock(DataInputStream in) throws IOException
	{
		...
		if (datanode.isBlockTokenEnabled)
		{
			try
			{
				datanode.blockTokenSecretManager.checkAccess(accessToken, null,
			...
			catch (InvalidToken e)
			{
				try
				{
					...
					throw new IOException(
							"Access token verification failed, for client "
									+ remoteAddress
		...
		try
		{
			...
			{
				// If client verification succeeded, and if it's for the whole
				// block,
				// tell the DataBlockScanner that it's good. This is an optional
				// response
				// from client. If absent, we close the connection (which is
				// what we
				// always do anyways).
				try
				{
	...
	private void writeBlock(DataInputStream in) throws IOException
	{
		...
		if (datanode.isBlockTokenEnabled)
		{
			...
			{
				datanode.blockTokenSecretManager.checkAccess(accessToken, null,
						block, BlockTokenSecretManager.AccessMode.WRITE);
			}
			catch (InvalidToken e)
			{
				try
			...
			if (targets.length > 0)
			{
				...
				try
				{
					...
					if (client.length() != 0)
					{
						...
						if (LOG.isDebugEnabled()
								|| mirrorInStatus != DataTransferProtocol.OP_STATUS_SUCCESS)
						{
							LOG.info("Datanode "
									+ targets.length
				...
				catch (IOException e)
				{
					...
					{
						LOG.info(datanode.dnRegistration
	...
	private void copyBlock(DataInputStream in) throws IOException
	{
		...
		if (datanode.isBlockTokenEnabled)
		{
			...
			{
				datanode.blockTokenSecretManager.checkAccess(accessToken, null,
						block, BlockTokenSecretManager.AccessMode.COPY);
			}
			catch (InvalidToken e)
			{
	...
	private void replaceBlock(DataInputStream in) throws IOException
	{
		...
		if (datanode.isBlockTokenEnabled)
		{
			...
			{
				datanode.blockTokenSecretManager.checkAccess(accessToken, null,
						block, BlockTokenSecretManager.AccessMode.REPLACE);
			}
			catch (InvalidToken e)
			{
				LOG.warn("Invalid access token in request from "
		...
		try
		{
			...
			blockReceiver = new BlockReceiver(block, proxyReply, proxySock
					...
					.getLocalSocketAddress().toString(), false, "", null,
					datanode);
