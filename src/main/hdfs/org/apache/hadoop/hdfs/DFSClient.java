...
public class DFSClient implements FSConstants, java.io.Closeable {
  ...
  /**
   * We assume we're talking to another CDH server, which supports HDFS-630's
   * addBlock method. If we get a RemoteException indicating it doesn't, we'll
   * set this false and stop trying.
   */
  private volatile boolean serverSupportsExcludedBlockApi = true;

  /**
   * We assume we're talking to another CDH3u3+ server, which supports
   * HDFS-617's non-recursive-create method. If we get a RemoteException
   * indicating it doesn't, we warn and set this flag falling back to using a
   * client-side checking version.
   */
  ...
  private static ClientProtocol createRPCNamenode(
  ...
  private static ClientProtocol createNamenode(ClientProtocol rpcNamenode)
    ...
    RetryPolicy createPolicy = RetryPolicies
        .retryUpToMaximumCountWithFixedSleep(5, LEASE_SOFTLIMIT_PERIOD,
  ...
  static ClientDatanodeProtocol createClientDatanodeProtocolProxy(
    ...
    UserGroupInformation ticket = UserGroupInformation.createRemoteUser(block
        .toString());
    ticket.addToken(token);
    return (ClientDatanodeProtocol) RPC.getProxy(ClientDatanodeProtocol.class,
        ClientDatanodeProtocol.versionID, addr, ticket, conf,
   ...
   * 
   * @see #DFSClient(InetSocketAddress, ClientProtocol, Configuration,
   *      org.apache.hadoop.fs.FileSystem.Statistics)
   */
  public DFSClient(InetSocketAddress nameNodeAddr, Configuration conf,
  ...
  DFSClient(InetSocketAddress nameNodeAddr, ClientProtocol rpcNamenode,
    ...
    this.datanodeWriteTimeout = conf.getInt(
        "dfs.datanode.socket.write.timeout", HdfsConstants.WRITE_TIMEOUT);
    ...
    this.socketCache = new SocketCache(conf.getInt(
  ...
  public Token<DelegationTokenIdentifier> getDelegationToken(Text renewer)
      throws IOException {
    Token<DelegationTokenIdentifier> result = namenode
  ...
  public BlockLocation[] getBlockLocations(String src, long start, long length)
    ...
    for (LocatedBlock blk : blocks.getLocatedBlocks()) {
      ...
      blkLocations[idx] = new BlockLocation(names, hosts, racks,
          blk.getStartOffset(), blk.getBlockSize());
      idx++;
    }
    return blkLocations;
  }
  ...
  /**
   * Create a new dfs file and return an output stream for writing into it with
  ...
  public OutputStream create(String src, boolean overwrite, short replication,
    ...
    return create(src, FsPermission.getDefault(), overwrite, replication,
  ...
  public OutputStream create(String src, FsPermission permission,
    ...
    return create(src, permission, overwrite, true, replication, blockSize,
        progress, buffersize);
  }
  ...
  public static MD5MD5CRC32FileChecksum getFileChecksum(String src,
    ...
    List<LocatedBlock> locatedblocks = callGetBlockLocations(namenode, src, 0,
      ...
      for (int j = 0; !done && j < datanodes.length; j++) {
        ...
        try {
          ...
          if (LOG.isDebugEnabled()) {
            ...
            LOG.debug("got reply from " + datanodes[j].getName() + ": md5="
                + md5);
          }
        } catch (IOException ie) {
  ...
  /**
   * Save namespace image. See {@link ClientProtocol#saveNamespace()} for more
   * details.
   * 
  ...
  void setQuota(String src, long namespaceQuota, long diskspaceQuota)
    ...
    if ((namespaceQuota <= 0 && namespaceQuota != FSConstants.QUOTA_DONT_SET && namespaceQuota != FSConstants.QUOTA_RESET)
      ...
      throw new IllegalArgumentException("Invalid values for quota : "
          + namespaceQuota + " and " + diskspaceQuota);
   ...
   * offset etc
   */
  public static class RemoteBlockReader extends FSInputChecker implements
      BlockReader {
    ...
    public boolean seekToNewSource(long targetPos) throws IOException {
       ...
       * does not always call 'seekToNewSource'. In the case of pread(), it just
       * tries a different replica without seeking.
       */
      return false;
    }
    ...
    private void adjustChecksumBytes(int dataLen) {
      int requiredSize = ((dataLen + bytesPerChecksum - 1) / bytesPerChecksum)
          * checksumSize;
      ...
      if (dataLeft <= 0) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug("DFSClient readChunk got seqno " + seqno
              + " offsetInBlock " + offsetInBlock + " lastPacketInBlock "
              + lastPacketInBlock + " packetLen " + packetLen);
        }
        ...
        if (dataLen < 0
          ...
          throw new IOException("BlockReader: error in packet header"
              + "(chunkOffset : " + chunkOffset + ", dataLen : " + dataLen
              + ", seqno : " + seqno + " (last: " + lastSeqNo + "))");
        }
    ...
    private RemoteBlockReader(String file, long blockId, DataInputStream in,
        DataChecksum checksum, boolean verifyChecksum, long startOffset,
        long firstChunkOffset, Socket dnSock) {
      super(new Path("/blk_" + blockId + ":of:" + file)/*
                                                        * too non path-like?
                                                        */, 1, verifyChecksum,
          checksum.getChecksumSize() > 0 ? checksum : null, checksum
      ...
      this.isLocal = DFSClient.isLocalAddress(new InetSocketAddress(dnSock
          .getInetAddress(), dnSock.getPort()));
    }
    ...
    protected RemoteBlockReader(Path file, int numRetries,
        DataChecksum checksum, boolean verifyChecksum) {
      super(file, numRetries, verifyChecksum,
          checksum.getChecksumSize() > 0 ? checksum : null, checksum
              .getBytesPerChecksum(), checksum.getChecksumSize());
      this.isLocal = true;
    }
     ...
     * 
     * @param sock
     *          Socket to read the block.
     * @param file
     *          File to which this block belongs.
     * @param blockId
     *          Block id.
     * @param accessToken
     *          Block access token.
     ...
     *          Checksum verification is required or not.
     ...
     * 
     * @param sock
     *          An established Socket to the DN. The BlockReader will not close
     *          it normally
     * @param file
     *          File location
     * @param block
    ...
    public static BlockReader newBlockReader(Socket sock, String file,
      ...
      DataInputStream in = new DataInputStream(new BufferedInputStream(
          NetUtils.getInputStream(sock), bufferSize));
      ...
      if (status != DataTransferProtocol.OP_STATUS_SUCCESS) {
        if (status == DataTransferProtocol.OP_STATUS_ERROR_ACCESS_TOKEN) {
          ...
          throw new IOException("Got error in response to OP_READ_BLOCK "
              + "self=" + sock.getLocalSocketAddress() + ", remote="
              + sock.getRemoteSocketAddress() + " for file " + file
              + " for block " + blockId + "_" + genStamp);
        }
      }
    ...
    /**
     * kind of like readFully(). Only reads as much as possible. And allows use
     * of protected readFully().
     */
    ...
    /**
     * Whether the BlockReader has reached the end of its input stream and
     * successfully sent a status code back to the datanode.
     */
    ...
    void sendReadResult(Socket sock, int statusCode) {
      ...
      try {
        OutputStream out = NetUtils.getOutputStream(sock,
        ...
        // It's ok not to be able to send this. But something is
        // probably wrong.
  ...
  public static class ReadStatistics {    
    ...
    private long periodBytesRead;

    private long periodLocalBytesRead;

    private long periodShortCircuitBytesRead;
    ...
    public long getPeriodBytesRead() {
      return periodBytesRead;
    }

    public long getPeriodLocalBytesRead() {
      return periodLocalBytesRead;
    }

    public long getPeriodShortCircuitBytesRead() {
      return periodShortCircuitBytesRead;
    }


    synchronized public void updateReadStatistics(int len, BlockReader reader) {
      if (len <= 0)
        return;
      if (reader.isShortCircuit()) {
        totalBytesRead += len;
        totalLocalBytesRead += len;
        totalShortCircuitBytesRead += len;

        periodBytesRead += len;
        periodLocalBytesRead += len;
        periodShortCircuitBytesRead += len;
    ...
    synchronized public void resetPeriodValues(){
      periodBytesRead = 0;
      periodLocalBytesRead = 0;
      periodShortCircuitBytesRead = 0;
    }
    
    synchronized public void addValues(ReadStatistics toAddIn){
      toAddIn.totalBytesRead += totalBytesRead;
      toAddIn.totalLocalBytesRead += totalLocalBytesRead;
      toAddIn.totalShortCircuitBytesRead += totalShortCircuitBytesRead;
      toAddIn.periodBytesRead += periodBytesRead;
      toAddIn.periodLocalBytesRead += periodLocalBytesRead;
      toAddIn.periodShortCircuitBytesRead += periodShortCircuitBytesRead;
    }
  }
  ...
  public class DFSInputStream extends FSInputStream {
    ...
    synchronized void openInfo() throws IOException {
      ...
      // I think this check is not correct. A file could have been
      // appended to
        ...
        if (lastBlockInFile && last.getLocations().length > 0) {
          ...
          try {
            primary = createClientDatanodeProtocolProxy(primaryNode, conf,
                DFSClient.this.socketTimeout, last.getBlock(),
                last.getBlockToken());
    ...
    private synchronized DatanodeInfo blockSeekTo(long target)
      ...
      while (true) {
        ...
        try {
          blockReader = getBlockReader(targetAddr, src, blk.getBlockId(),
              accessToken, blk.getGenerationStamp(), offsetIntoBlock,
              blk.getNumBytes() - offsetIntoBlock, buffersize, verifyChecksum,
              clientName);
          return chosenNode;
        } catch (IOException ex) {
          if (refetchToken > 0 && tokenRefetchNeeded(ex, targetAddr)) {
    ...
    private synchronized int readBuffer(byte buf[], int off, int len)
      ...
      while (true) {
        ...
        try {
          ...
          if (!retryCurrentNode) {
            LOG.warn("Exception while reading from " + currentBlock + " of "
                + src + " from " + currentNode + ": "
        ...
        if (retryCurrentNode) {
          /*
           * possibly retry the same node so that transient errors don't result
           * in application level failures (e.g. Datanode could have closed the
           * connection because the client is idle for too long).
           */
      ...
      if (pos < getFileLength()) {
        ...
        while (retries > 0) {
          try {
            ...
            if (readStatistics != null && result != -1) {
              readStatistics.updateReadStatistics(result, blockReader);
            }
            return result;
          } catch (ChecksumException ce) {
            throw ce;
    ...
    private void fetchBlockByteRange(LocatedBlock block, long start, long end,
      ...
      while (true) {
        // cached block locations may have been updated by
        // chooseDataNode()
        // or fetchBlockAt(). Always get the latest list of locations at
        // the
        // start of the loop.
        block = getBlockAt(block.getStartOffset(), false);
        ...
        try {
          ...
          if (shouldTryShortCircuitRead(targetAddr)) {
            ...
            // go to the datanode
            reader = getBlockReader(targetAddr, src, block.getBlock()
                .getBlockId(), accessToken, block.getBlock()
                .getGenerationStamp(), start, len, buffersize, verifyChecksum,
                clientName);
            IOUtils.readFully((RemoteBlockReader) reader, buf, offset, len);
          }
          if (readStatistics != null && len > 0) {
            readStatistics.updateReadStatistics(len, reader);
          }
          return;
        } catch (ChecksumException e) {
          LOG.warn("fetchBlockByteRange(). Got a checksum exception for " + src
              + " at " + block.getBlock() + ":" + e.getPos() + " from "
              + chosenNode.getName());
    ...
    /**
     * Retrieve a BlockReader suitable for reading. This method will reuse the
     * cached connection to the DN if appropriate. Otherwise, it will create a
     * new connection.
     * 
     * @param dnAddr
     ...
     */
    protected BlockReader getBlockReader(InetSocketAddress dnAddr, String file,
      ...
      // Allow retry since there is no way of knowing whether the cached
      // socket
      ...
      for (int retries = 0; retries <= nCachedConnRetry && fromCache; ++retries) {
        ...
        if (sock == null) {
          ...
          // TCP_NODELAY is crucial here because of bad interactions
          // between
          // Nagle's Algorithm and Delayed ACKs. With connection
          // keepalive
          ...
          // The fact that step #3 and #4 are both in the client->DN
          // direction
          // triggers Nagling. If the DN is using delayed ACKs, this
          // results
          // in a delay of 40ms or more.
          //
          // TCP_NODELAY disables nagling and thus avoids this
          // performance
          // disaster.
        ...
        try {
          // The OP_READ_BLOCK request is sent as we make the
          // BlockReader
          BlockReader reader = RemoteBlockReader.newBlockReader(sock, file,
              blockId, blockToken, genStamp, startOffset, len, bufferSize,
              verifyChecksum, clientName);
          return reader;
        } catch (IOException ex) {
          // Our socket is no good.
          DFSClient.LOG.debug(
              "Error making BlockReader. Closing stale " + sock, ex);
          sock.close();
          err = ex;
        }
      }
    ...
    public int read(long position, byte[] buffer, int offset, int length)
      ...
      for (LocatedBlock blk : blockRange) {
        ...
        long bytesToRead = Math
            .min(remaining, blk.getBlockSize() - targetStart);
    ...
    public synchronized boolean seekToNewSource(long targetPos)
      ...
      if (!markedDead) {
        /*
         * remove it from deadNodes. blockSeekTo could have cleared deadNodes
         * and added currentNode again. Thats ok.
         */
    ...
    /**
     * After hdfs block balanced, client should refresh its blocks' location
     * 
     * @throws IOException
     */
    public synchronized void refreshLocatedBlocks() throws IOException {
      // Reinit the locatedBlocks, block Locations will be asked
      // from namenode is missing
      openInfo();
    }
  }
   ...
   * datanodes, the ResponseProcessor removes the corresponding packet from the
   * ackQueue.
   * 
  ...
  class DFSOutputStream extends FSOutputSummer implements Syncable {
            ...
            try {
              ...
              if (offsetInBlock >= blockSize) {
                throw new IOException("BlockSize " + blockSize
                    ...
                    + " Offset of packet in block " + offsetInBlock
                    + " Aborting file " + src);
              }
              ...
              if (LOG.isDebugEnabled()) {
                LOG.debug("DataStreamer block " + block
                    + " wrote packet seqno:" + one.seqno + " size:"
        ...
        while (!closed && clientRunning && !lastPacketInBlock) {
          ...
          try {
            ...
            if (seqno == PipelineAck.HEART_BEAT.getSeqno()) {
              ...
              // This signifies that some pipeline node failed to
              // read downstream
              // and therefore has no idea what sequence number
              ...
              if (one.seqno != seqno) {
                throw new IOException("Responseprocessor: Expecting seqno "
                    + " for block " + block + one.seqno + " but received "
                    + seqno);
              }
              lastPacketInBlock = one.lastPacketInBlock;
            }
            ...
            if (!closed) {
              ...
              LOG.warn("DFSOutputStream ResponseProcessor exception "
                  + " for block " + block + StringUtils.stringifyException(e));
              closed = true;
              streamer.interrupt();
            }
          }
    ...
    private boolean processDatanodeError(boolean hasError, boolean isAppend) {
      ...
      if (errorIndex >= 0) {
        LOG.warn("Error Recovery for block " + block + " bad datanode["
            + errorIndex + "] "
      ...
      while (!success && clientRunning) {
        ...
        if (errorIndex < 0) {
          ...
          LOG.warn("Error Recovery for block " + block + " in pipeline "
              + pipelineMsg + ": bad datanode " + nodes[errorIndex].getName());
        ...
        try {
          ...
          // Set the timeout to reflect that recovery requires at most
          // two rpcs
          // to each DN and two rpcs to the NN.
          int recoveryTimeout = (newnodes.length * 2 + 2)
              * DFSClient.this.socketTimeout;
          primary = createClientDatanodeProtocolProxy(primaryNode, conf,
              recoveryTimeout, block, accessToken);
          ...
          int maxRecoveryErrorCount = conf.getInt(
              "dfs.client.block.recovery.retries", 5);
          ...
          LOG.warn("Error Recovery for block " + block + " failed "
              + " because recovery from primary datanode " + primaryNode
              + " failed " + recoveryErrorCount + " times. " + " Pipeline was "
              + pipelineMsg + ". Will retry...");
          return true; // sleep when we return from here
        } finally {
    ...
    DFSOutputStream(String src, FsPermission masked, boolean overwrite,
      ...
      try {
        if (!createParent && serverSupportsNonRecursiveCreateApi) {
          // Compatibility case: createParent is false and we think we
          // support
          // the call
          namenode.create(src, masked, clientName, overwrite, createParent,
              replication, blockSize);
        } else {
          // if createParent=true it doesn't matter if we support the
          // alternate
          // create method
        ...
        IOException ioe = re.unwrapRemoteException(
            AccessControlException.class, FileAlreadyExistsException.class,
            FileNotFoundException.class, NSQuotaExceededException.class,
            DSQuotaExceededException.class);
     ...
     */
    private void retryCreate(String src, FsPermission masked,
        boolean overwrite, short replication, long blockSize,
        Progressable progress, int buffersize, int bytesPerChecksum)
        throws IOException {
      // The call was invalid, warn and then try the version that will
      // work
      // an older name node.
    ...
    DFSOutputStream(String src, int buffersize, Progressable progress,
        LocatedBlock lastBlock, HdfsFileStatus stat, int bytesPerChecksum)
        throws IOException {
      this(src, stat.getBlockSize(), progress, bytesPerChecksum, stat
          .getReplication());
      ...
      if (lastBlock != null) {
        ...
        // keep trying to setup a pipeline until you know all DNs are
        // dead
    ...
    private void computePacketChunkSize(int psize, int csize) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug("computePacketChunkSize: src=" + src + ", chunkSize="
    ...
    private boolean createBlockOutputStream(DatanodeInfo[] nodes,
        String client, boolean recoveryFlag) {
      ...
      try {
        ...
        InetSocketAddress target = NetUtils
            .createSocketAddr(nodes[0].getName());
        ...
        long writeTimeout = (datanodeWriteTimeout > 0) ? (HdfsConstants.WRITE_TIMEOUT_EXTENSION
        ...
        while (true) {
          try {
            ...
            if (e
              ...
              // We're talking to a server that doesn't implement
              // HDFS-630.
    ...
    public void sync() throws IOException {
      try {
        ...
        synchronized (this) {
           ...
           * value, any partial checksum chunk will be sent now and in next
           * packet.
           */
          ...
          flushBuffer(true);
          // bytesCurBlock potentially incremented if there was
          // buffered data
          ...
          // Flush only if we haven't already flushed till this
          // offset.
          if (lastFlushOffset != bytesCurBlock) {
            ...
            if (oldCurrentPacket == null && currentPacket != null) {
              // If we didn't previously have a packet queued, and
              // now we do,
              // but we don't plan on sending it, then we should
              // not
        ...
        if (willPersist) {
          try {
            ...
            IOException unwrapped = re
                .unwrapRemoteException(LeaseExpiredException.class);
            if (unwrapped instanceof LeaseExpiredException) {
              // This situation is actually normal in the case of
              // concurrent sync()
              // and close(). The close() will do the flush and
              // close the lease on the
              // NN. Thus wewhen we try to persist, the file will
              // already be closed,
              // and we can ignore the error because the close()
              // already persisted
              // the blocks!
    ...
    /**
     * Harsh abort method that should only be used from tests - this is in order
     * to prevent pipeline recovery when eg a DN shuts down.
     */
