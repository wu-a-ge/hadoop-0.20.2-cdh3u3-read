...
public class DistributedFileSystem extends FileSystem
{
	...
	{
		return new Path("/user/" + dfs.ugi.getShortUserName())
				.makeQualified(this);
	}
	...
	{
		statistics.incrementReadOps(1);
		return new DFSClient.DFSDataInputStream(dfs.open(getPathName(f),
				bufferSize, verifyChecksum, statistics));
	}
	 ...
	 *             if server does not support this function
	 */
			...
			throws IOException
	{
		...
		return new FSDataOutputStream(
				dfs.create(getPathName(f), permission, overwrite, false,
	...
	private FileStatus makeQualified(HdfsFileStatus f, Path parent)
	{
		return new FileStatus(f.getLen(), f.isDir(), f.getReplication(),
				...
				f.getPermission(), f.getOwner(), f.getGroup(), f.getFullPath(
						parent).makeQualified(this)); // fully-qualify path
	}
	...
	public FileStatus[] listStatus(Path p) throws IOException
	{
		...
		DirectoryListing thisListing = dfs.listPaths(src,
		...
		int totalNumEntries = partialListing.length
				+ thisListing.getRemainingEntries();
		ArrayList<FileStatus> listing = new ArrayList<FileStatus>(
				totalNumEntries);
			...
			long inPos, FSDataInputStream sums, long sumsPos)
	{
		...
		DatanodeInfo[] dataNode = {
			dfsIn.getCurrentDatanode()
		};
		lblocks[0] = new LocatedBlock(dataBlock, dataNode);
		LOG.info("Found checksum error in data stream at block=" + dataBlock
		...
		DatanodeInfo[] sumsNode = {
			dfsSums.getCurrentDatanode()
		};
	...
	/**
	 * Delegation Token Operations These are DFS only operations.
	 */
