...
{
	public static final Log LOG = LogFactory.getLog(SchedulingAlgorithms.class
			.getName());
	...
	public static class FairShareComparator implements Comparator<Schedulable>
	{
		...
		public int compare(Schedulable s1, Schedulable s2)
		{
			...
			if (s1Needy && !s2Needy)
				...
				res = (int) Math.signum(tasksToWeightRatio1
						- tasksToWeightRatio2);
			if (res == 0)
			{
				// Jobs are tied in fairness ratio. Break the tie by submit time
				// and job
	 ...
	 * equivalent to the number of bits of precision in the output. 25
	 * iterations gives precision better than 0.1 slots in clusters with one
	 * million slots.
	 */
	 ...
	 * fair shares. The min shares and demands of the Schedulables are assumed
	 * to be set beforehand. We compute the fairest possible allocation of
	 * shares to the Schedulables that respects their min shares and demands.
	 * 
	 * To understand what this method does, we must first define what weighted
	 * fair sharing means in the presence of minimum shares and demands. If
	 * there were no minimum shares and every Schedulable had an infinite demand
	 * (i.e. could launch infinitely many tasks), then weighted fair sharing
	 * would be achieved if the ratio of slotsAssigned / weight was equal for
	 * each Schedulable and all slots were assigned. Minimum shares and demands
	 * add two further twists: - Some Schedulables may not have enough tasks to
	 * fill all their share. - Some Schedulables may have a min share higher
	 * than their assigned share.
	 * 
	 ...
	 * S.demand < R * S.weight are assigned share S.demand - Schedulables S
	 * where S.minShare > R * S.weight are given share S.minShare - All other
	 ...
	 * until we either use more than totalSlots slots or we fulfill all jobs'
	 * demands). The helper method slotsUsedWithWeightToSlotRatio computes the
	 * total number of slots used with a given value of R.
	 * 
	 * The running time of this algorithm is linear in the number of
	 * Schedulables, because slotsUsedWithWeightToSlotRatio is linear-time and
	 * the number of iterations of binary search is a constant (dependent on
	 * desired precision).
	 */
			...
			Collection<? extends Schedulable> schedulables, double totalSlots)
	{
		...
		// start
		// at R = 1 and double it until we have either used totalSlots slots or
		// we
