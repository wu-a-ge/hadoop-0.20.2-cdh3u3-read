...
public class FairScheduler extends TaskScheduler
{
	public static final Log LOG = LogFactory
			.getLog("org.apache.hadoop.mapred.FairScheduler");
	...
	private static final TaskType[] MAP_AND_REDUCE = new TaskType[] {
	...
	protected long lastPreemptionUpdateTime; // Time when we last updated
												// preemption vars
	...
	protected boolean autoComputeLocalityDelay = false; // Compute locality
														// delay
														// from heartbeat
														// interval
	...
	private boolean mockMode; // Used for unit tests; disables background
								// updates
								// and scheduler event log
	public FairSchedulerEventLog eventLog;
	...
	private long lastPreemptCheckTime; // Time we last ran
	 ...
	 */
	public final static String ALLOW_UNDECLARED_POOLS_KEY = "mapred.fairscheduler.allow.undeclared.pools";
	private boolean allowUndeclaredPools = false;

	private MetricsUpdater metricsUpdater; // responsible for pushing hadoop
											// metrics
	...
	static class JobInfo
	{
		...
		LocalityLevel lastMapLocalityLevel; // Locality level of last map
											// launched
		long timeWaitedForLocalMap; // Time waiting for local map since last map
		boolean skippedAtLastHeartbeat; // Was job skipped at previous
										// assignTasks?
										// (used to update
										// timeWaitedForLocalMap)
	...
	public void start()
	{
		try
		{
			...
			if (!mockMode && logEnabled)
			{
				...
				{
					hostname = ((JobTracker) taskTrackerManager)
			...
			taskSelector = (TaskSelector) ReflectionUtils.newInstance(conf
					.getClass("mapred.fairscheduler.taskselector",
							DefaultTaskSelector.class, TaskSelector.class),
					conf);
			...
			dumpInterval = conf.getLong("mapred.fairscheduler.dump.interval",
					10000);
			...
			localityDelay = conf.getLong("mapred.fairscheduler.locality.delay",
					-1);
			allowUndeclaredPools = conf.getBoolean(ALLOW_UNDECLARED_POOLS_KEY,
					true);
			if (localityDelay == -1)
				autoComputeLocalityDelay = true; // Compute from heartbeat
													// interval
	...
	/**
	 * Register metrics for the fair scheduler, and start a thread to update
	 * them periodically.
	 */
	...
	private class JobListener extends JobInProgressListener
	{
		...
		public void jobAdded(JobInProgress job)
		{
			synchronized (FairScheduler.this)
			{
				...
				JobInfo info = new JobInfo(new JobSchedulable(
						FairScheduler.this, job, TaskType.MAP),
						new JobSchedulable(FairScheduler.this, job,
								TaskType.REDUCE));
				infos.put(job, info);
				poolMgr.addJob(job); // Also adds job into the right
										// PoolScheduable
				update();
			}
		}
			...
			throws IOException
	{
		...
		eventLog.log("RUNNABLE_TASKS", runnableMaps, runningMaps,
				runnableReduces, runningReduces);
		...
		// Keep track of which jobs were visited for map tasks and which had
		// tasks
		...
		// Scan jobs to assign tasks until neither maps nor reduces can be
		// assigned
		while (true)
		{
			...
			if (!mapRejected)
			{
				...
				{
					eventLog.log("INFO", "Can't assign another MAP to "
							+ trackerName);
					mapRejected = true;
				}
			}
			if (!reduceRejected)
			{
				...
				{
					eventLog.log("INFO", "Can't assign another REDUCE to "
							+ trackerName);
					reduceRejected = true;
				}
			}
			...
			if (mapRejected && reduceRejected || !assignMultiple
					&& tasks.size() > 0)
			{
			...
			{
				// If both types are available, choose the task type with fewer
				// running
				// tasks on the task tracker to prevent that task type from
				// starving
				if (tts.countMapTasks() + mapsAssigned <= tts
						.countReduceTasks() + reducesAssigned)
				{
					taskType = TaskType.MAP;
				}
				else
			...
			Collections.sort(scheds,
					new SchedulingAlgorithms.FairShareComparator());
			boolean foundTask = false;
			for (Schedulable sched : scheds)
			{ // This loop will assign only one task
				eventLog.log("INFO", "Checking for " + taskType + " task in "
						+ sched.getName());
				Task task = taskType == TaskType.MAP ? sched.assignTask(tts,
						currentTime, visitedForMap) : sched.assignTask(tts,
						currentTime, visitedForReduce);
				if (task != null)
				{
					foundTask = true;
					JobInProgress job = taskTrackerManager.getJob(task
							.getJobID());
		...
		// Mark any jobs that were visited for map tasks but did not launch a
		// task
	...
	private int maxTasksToAssign(TaskType type, TaskTrackerStatus tts)
	{
		...
		if (cap == -1) // Infinite cap; use the TaskTracker's slot count
			return (type == TaskType.MAP) ? tts.getAvailableMapSlots() : tts
	 ...
	 * tasks, based on how long it has been waiting for local tasks. This is
	 * used to implement the "delay scheduling" feature of the Fair Scheduler
	 * for optimizing data locality. If the job has no locality information
	 * (e.g. it does not use HDFS), this method returns LocalityLevel.ANY,
	 * allowing tasks at any level. Otherwise, the job can only launch tasks at
	 * its current locality level or lower, unless it has waited at least
	 * localityDelay milliseconds (in which case it can go one level beyond) or
	 * 2 * localityDelay millis (in which case it can go to any level).
	 */
			...
			long currentTime)
	{
		JobInfo info = infos.get(job);
		if (info == null)
		{ // Job not in infos (shouldn't happen)
		...
		{ // Job doesn't have locality information
			return LocalityLevel.ANY;
		}
		...
		{
			eventLog.log("INFO", "No delay scheduling for " + job.getJobID()
					+ " because it is being starved");
			return LocalityLevel.ANY;
		}
	...
	protected void update()
	{
		synchronized (taskTrackerManager)
		{
			synchronized (this)
			{
				ClusterStatus clusterStatus = taskTrackerManager
						.getClusterStatus();

				// Recompute locality delay from JobTracker heartbeat interval
				// if enabled.
				// This will also lock the JT, so do it outside of a fair
				// scheduler lock.
				if (autoComputeLocalityDelay)
				{
					JobTracker jobTracker = (JobTracker) taskTrackerManager;
					localityDelay = Math
							.min(MAX_AUTOCOMPUTED_LOCALITY_DELAY,
									(long) (1.5 * jobTracker
				...
				updateRunnability(); // Set job runnability based on user/pool
										// limits
				...
				SchedulingAlgorithms.computeFairShares(mapScheds,
						clusterStatus.getMaxMapTasks());
				SchedulingAlgorithms.computeFairShares(reduceScheds,
	...
	private void updateRunnability()
	{
		...
		for (JobInProgress job : jobs)
		{
			if (job.getStatus().getRunState() == JobStatus.RUNNING)
			{
				...
				int userCount = userJobs.containsKey(user) ? userJobs.get(user)
						: 0;
				int poolCount = poolJobs.containsKey(pool) ? poolJobs.get(pool)
						: 0;
	...
	public double getJobWeight(JobInProgress job, TaskType taskType)
	{
		if (!isRunnable(job))
		{
			// Job won't launch tasks, but don't return 0 to avoid division
			// errors
			return 1.0;
		}
		else
		{
			...
			if (sizeBasedWeight)
			{
				...
				JobInfo info = infos.get(job);
				int runnableTasks = (taskType == TaskType.MAP) ? info.mapSchedulable
						.getDemand() : info.reduceSchedulable.getDemand();
	...
	{
		return (type == TaskType.MAP ? clusterStatus.getMaxMapTasks()
				: clusterStatus.getMaxReduceTasks());
	}

	/**
	 * Update the preemption fields for all PoolScheduables, i.e. the times
	 * since each pool last was at its guaranteed share and at > 1/2 of its fair
	 * share for each type of task.
	 * 
	...
	/**
	 * Is a pool being starved for fair share for the given task type? This is
	 ...
	 * such pools exist, compute how many tasks of each type need to be
	...
	/**
	 * Preempt a given number of tasks from a list of PoolSchedulables. The
	 * policy for this is to pick tasks from pools that are over their fair
	 ...
	 * two amounts (this shouldn't happen unless someone sets the timeouts to be
	...
	protected int tasksToPreempt(PoolSchedulable sched, long curTime)
	{
		...
		{
			int target = (int) Math
					.min(sched.getFairShare(), sched.getDemand());
		...
		if (tasksToPreempt > 0)
		{
			String message = "Should preempt " + tasksToPreempt + " "
					+ sched.getTaskType() + " tasks for pool "
					+ sched.getName() + ": tasksDueToMinShare = "
					+ tasksDueToMinShare + ", tasksDueToFairShare = "
					+ tasksDueToFairShare;
	...
	private List<TaskStatus> getRunningTasks(JobInProgress job, TaskType type)
	{
		...
		if (type == TaskType.MAP)
		{
			...
			// locality
			// info (e.g. the input file is not in HDFS), and maps with locality
			// info,
			// which are stored in the runningMapCache map from location to task
			// list
		...
		// more than one if the task has multiple copies active due to
		// speculation)
		...
		for (TaskInProgress tip : tips)
		{
			for (TaskAttemptID id : tip.getActiveTasks().keySet())
			{
				...
				// status is null when the task has been scheduled but not yet
				// running
				if (stat != null)
				{
					statuses.add(stat);
	...
	private synchronized void dump()
	{
		synchronized (eventLog)
		{
			...
			Collections.sort(jobs, new Comparator<JobInProgress>()
			{
				...
				{
					return (int) Math.signum(j1.getStartTime()
							- j2.getStartTime());
				}
			});
			...
			for (JobInProgress job : jobs)
			{
				...
				eventLog.log("JOB", profile.getJobID(), profile.name,
						profile.user, job.getPriority(),
						poolMgr.getPoolName(job), job.numMapTasks,
						ms.getRunningTasks(), ms.getDemand(),
						ms.getFairShare(), ms.getWeight(), job.numReduceTasks,
						rs.getRunningTasks(), rs.getDemand(),
			...
			for (Pool pool : pools)
			{
				...
				eventLog.log("POOL", name, poolMgr.getPoolWeight(name), pool
						.getJobs().size(), poolMgr.getAllocation(name,
						TaskType.MAP), runningMaps, poolMgr.getAllocation(name,
						TaskType.REDUCE), runningReduces);
			}
	...
	/**
	 * Examines the job's pool name to determine if it is a declared pool name
	 * (in the scheduler allocation file).
	 */
	@Override
			...
			throws UndeclaredPoolException
	{
		...
		if (!this.allowUndeclaredPools
			...
			throw new UndeclaredPoolException(
					"Pool name: '"
