...
public class PoolSchedulable extends Schedulable
{
	...
	public void addJob(JobInProgress job)
	{
		...
		jobScheds.add(taskType == TaskType.MAP ? info.mapSchedulable
				: info.reduceSchedulable);
	}
			...
			Collection<JobInProgress> visited) throws IOException
	{
		...
		{
			throw new RuntimeException("Unsupported pool scheduling mode "
					+ mode);
		}
	...
	public void updateMetrics()
	{
		...
		if (scheduler.isPreemptionEnabled())
		{
			...
			metrics.setMetric("millisSinceAtMinShare", lastCheck
					- lastTimeAtMinShare);
			metrics.setMetric("millisSinceAtHalfFairShare", lastCheck
					- lastTimeAtHalfFairShare);
		}
		metrics.update();
