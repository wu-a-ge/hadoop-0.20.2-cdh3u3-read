...
class FairSchedulerEventLog
{
	private static final Log LOG = LogFactory
			.getLog("org.apache.hadoop.mapred.FairSchedulerEventLog");
	...
	/**
	 * Active log file, which is
	 * {LOG_DIR}/hadoop-{user}-fairscheduler.{host}.log. Older files are also
	 * stored as {LOG_FILE}.date (date format YYYY-MM-DD).
	 */
	...
	boolean init(Configuration conf, String jobtrackerHostname)
	{
		...
		{
			logDir = conf.get(
					"mapred.fairscheduler.eventlog.location",
		...
		{
			LOG.error(
					"Failed to initialize fair scheduler event log. Disabling it.",
					e);
			logDisabled = true;
		}
		return !(logDisabled);
	...
	synchronized void log(String eventType, Object... params)
	{
		try
		{
			...
			appender.append(new LoggingEvent("", logger, Level.INFO, message,
					null));
		}
		catch (Exception e)
		{
