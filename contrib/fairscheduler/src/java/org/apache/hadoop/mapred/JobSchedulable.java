...
public class JobSchedulable extends Schedulable
{
	...
	public void updateDemand()
	{
		...
		if (isRunnable())
		{
			// For reduces, make sure enough maps are done that reduces can
			// launch
			...
			// - have N attempts running, in which case it demands N slots, and
			// may
			// potentially demand one more slot if it needs to be speculated
			TaskInProgress[] tips = (taskType == TaskType.MAP ? job
					.getTasks(TaskType.MAP) : job.getTasks(TaskType.REDUCE));
			boolean speculationEnabled = (taskType == TaskType.MAP ? job
					.hasSpeculativeMaps() : job.hasSpeculativeReduces());
			...
			for (TaskInProgress tip : tips)
			{
				if (!tip.isComplete())
				{
					if (tip.isRunning())
					{
						// Count active tasks and any speculative task we want
						// to launch
						demand += tip.getActiveTasks().size();
						if (speculationEnabled
								&& tip.hasSpeculativeTask(time, job.getStatus()
										.mapProgress()))
							demand += 1;
					}
					else
	...
	{
		return taskType == TaskType.MAP ? job.runningMaps() : job
				.runningReduces();
	}
			...
			Collection<JobInProgress> visited) throws IOException
	{
		if (isRunnable())
		{
			...
			{
				LocalityLevel localityLevel = scheduler
						.getAllowedLocalityLevel(job, currentTime);
				scheduler.getEventLog().log("ALLOWED_LOC_LEVEL",
						job.getJobID(), localityLevel);
				// obtainNewMapTask needs to be passed 1 + the desired locality
				// level
				return job.obtainNewMapTask(tts, numTaskTrackers,
						ttm.getNumberOfUniqueHosts(),
						localityLevel.toCacheLevelCap());
			}
			else
			{
