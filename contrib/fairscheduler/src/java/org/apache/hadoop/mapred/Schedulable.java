...
/**
 * A Schedulable represents an entity that can launch tasks, such as a job or a
 * pool. It provides a common interface so that algorithms such as fair sharing
 * can be applied both within a pool and across pools. There are currently two
 * types of Schedulables: JobSchedulables, which represent a single job, and
 * PoolSchedulables, which allocate among jobs in their pool.
 * 
 ...
 * 
 * A Schedulable is responsible for three roles: 1) It can launch tasks through
 * assignTask(). 2) It provides information about the job/pool to the scheduler,
 * including: - Demand (maximum number of tasks required) - Number of currently
 * running tasks - Minimum share (for pools) - Job/pool weight (for fair
 * sharing) - Start time and priority (for FIFO) 3) It can be assigned a fair
...
abstract class Schedulable
{
	 ...
	 * number of currently running tasks + number of unlaunched tasks (tasks
	 * that are either not yet launched or need to be speculated).
	 */
	...
	/**
	 * Distribute the fair share assigned to this Schedulable among its children
	 * (used in pools where the internal scheduler is fair sharing).
	 */
	public abstract void redistributeShare();

	/**
	 * Obtain a task for a given TaskTracker, or null if the Schedulable has no
	 * tasks to launch at this moment or does not wish to launch a task on this
	 * TaskTracker (e.g. is waiting for a TaskTracker with local data). In
	 * addition, if a job is skipped during this search because it is waiting
	 ...
	 * {@link FairScheduler#getAllowedLocalityLevel(JobInProgress, long)} for
	 * details of delay scheduling (waiting for trackers with local data).
	 * 
	 * @param tts
	 *            TaskTracker that the task will be launched on
	 * @param currentTime
	 *            Cached time (to prevent excessive calls to gettimeofday)
	 * @param visited
	 *            A Collection to which this method must add all jobs that were
	 *            considered during the search for a job to assign.
	 * @return Task to launch, or null if Schedulable cannot currently launch
	 *         one.
	 * @throws IOException
	 *             Possible if obtainNew(Map|Reduce)Task throws exception.
	 */
	...
	public String toString()
	{
		return String.format("[%s, demand=%d, running=%d, share=%.1f, w=%.1f]",
				getName(), getDemand(), getRunningTasks(), fairShare,
				getWeight());
	}
}
