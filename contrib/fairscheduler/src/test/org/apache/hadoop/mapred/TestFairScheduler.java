...
public class TestFairScheduler extends TestCase
{
	final static String TEST_DIR = new File(System.getProperty(
			"test.build.data", "build/contrib/streaming/test/data"))
			.getAbsolutePath();
	...
	class FakeJobInProgress extends JobInProgress
	{
		...
		private final String[][] mapInputLocations; // Array of hosts for each
													// map
		...
		public synchronized void initTasks() throws IOException
		{
			...
			// cleanup map tip.
			cleanup[0] = new TaskInProgress(jobId, jobFile, null, jobtracker,
			...
			// setup map tip.
			setup[0] = new TaskInProgress(jobId, jobFile, null, jobtracker,
			...
			for (int i = 0; i < numMapTasks; i++)
			{
				...
				maps[i] = new FakeTaskInProgress(getJobID(), i, getJobConf(),
						this, inputLocations, split);
		...
		@Override
		public Task obtainNewMapTask(final TaskTrackerStatus tts,
				int clusterSize, int numUniqueHosts, int localityLevel)
				throws IOException
		{
			for (int map = 0; map < maps.length; map++)
			{
						...
						&& getLocalityLevel(tip, tts) < localityLevel)
				{
					...
					JobSplit.TaskSplitMetaInfo split = JobSplit.EMPTY_TASK_SPLIT;
					Task task = new MapTask("", attemptId, 0,
							split.getSplitIndex(), 1)
					{
						@Override
						public String toString()
						{
							return String.format("%s on %s", getTaskID(),
									tts.getTrackerName());
						}
					};
					runningMapTasks++;
				...
				int clusterSize, int ignored) throws IOException
		{
			...
			{
				FakeTaskInProgress tip = (FakeTaskInProgress) reduces[reduce];
				if (!tip.isRunning() && !tip.isComplete())
				{
					TaskAttemptID attemptId = getTaskAttemptID(tip);
	...
	class FakeTaskInProgress extends TaskInProgress
	{
		...
		{
			super(jId, "", jobConf.getNumMapTasks(), id, job.jobtracker,
					jobConf, job, 1);
			this.isMap = false;
			this.fakeJob = job;
			activeTasks = new TreeMap<TaskAttemptID, String>();
	...
	static class FakeTaskTrackerManager implements TaskTrackerManager
	{
		...
		private Map<String, FakeTaskInProgress> tips = new HashMap<String, FakeTaskInProgress>();
		...
		public FakeTaskTrackerManager(int numRacks, int numTrackersPerRack)
		{
			...
			for (int rack = 1; rack <= numRacks; rack++)
			{
				for (int node = 1; node <= numTrackersPerRack; node++)
				{
					...
					System.out.println("Creating TaskTracker tt" + id + " on "
							+ host);
		...
		public ClusterStatus getClusterStatus()
		{
			...
			return new ClusterStatus(numTrackers, maps, reduces, numTrackers
					* maxMapTasksPerTracker, numTrackers
		...
		public void reportTaskOnTracker(String trackerName, Task t)
		{
			...
			TaskTrackerStatus trackerStatus = trackers.get(trackerName)
					.getStatus();
			trackerStatus.getTaskReports()
					.add(tip.getTaskStatus(t.getTaskID()));
		}
		...
		public void finishTask(String taskTrackerName, String attemptId)
		{
			...
			trackers.get(taskTrackerName).getStatus().getTaskReports()
					.remove(status);
		}
			...
			boolean assignMultiple) throws IOException
	{
		...
		taskTrackerManager = new FakeTaskTrackerManager(numRacks,
				numNodesPerRack);
		clock = new FakeClock();
	...
	public void testAllocationFileParsing() throws Exception
	{
		...
		assertEquals(6, poolManager.getPools().size()); // 5 in file + default
														// pool
		assertEquals(0,
				poolManager.getAllocation(Pool.DEFAULT_POOL_NAME, TaskType.MAP));
		...
		assertEquals(120000,
				poolManager
						.getMinSharePreemptionTimeout(Pool.DEFAULT_POOL_NAME));
	...
	/**
	 * This test is identical to testSmallJobs but sets assignMultiple to true
	 * so that multiple tasks can be assigned per heartbeat.
	 */
	...
	public void testLargeJobs() throws IOException
	{
		...
		// Check that no new tasks can be launched once the tasktrackers are
		// full
		assertNull(scheduler.assignTasks(tracker("tt1")));
		assertNull(scheduler.assignTasks(tracker("tt2")));
		...
		// Check scheduler variables; the demands should now be 8 because 2
		// tasks
	 ...
	 * multiple tasks per heartbeat. Results should be the same as
	 * testLargeJobs.
	 */
	public void testLargeJobsWithAssignMultiple() throws IOException
	{
		...
		// the task since FakeJobInProgress does not properly maintain running
		// tasks, so the scheduler will always get an empty task list from
		// the JobInProgress's getTasks(TaskType.MAP)/getTasks(TaskType.REDUCE)
		// and
		// think they finished.
		taskTrackerManager.finishTask("tt1", "attempt_test_0001_m_000000_0");
		taskTrackerManager.finishTask("tt1", "attempt_test_0002_m_000000_0");
	...
	/**
	 * We submit two jobs such that one has 2x the priority of the other to a
	 * cluster of 3 nodes, wait for 100 ms, and check that the weights/shares
	...
	public void testJobsWithPriorities() throws IOException
	{
		...
		// Assign tasks and check that j2 gets 2x more tasks than j1. In
		// addition,
	 ...
	 * slots because it has an earlier start time. In a similar manner, reduces
	 * should be assigned as job2, job3, job2, job1.
	 */
	 ...
	 * tasks per heartbeat. Results should match
	 * testLargeJobsWithExcessCapacity.
	 */
	...
	public void testSizeBasedWeight() throws Exception
	{
		...
		assertTrue(scheduler.infos.get(job2).mapSchedulable.getFairShare() > scheduler.infos
				.get(job1).mapSchedulable.getFairShare());
		assertTrue(scheduler.infos.get(job1).reduceSchedulable.getFairShare() > scheduler.infos
				.get(job2).reduceSchedulable.getFairShare());
	}

	/**
	 * This test submits jobs in three pools: poolA, which has a weight of 2.0;
	 * poolB, which has a weight of 0.5; and the default pool, which should have
	 * a weight of 1.0. It then checks that the map and reduce fair shares are
	 * given out accordingly. We then submit a second job to pool B and check
	 * that each gets half of the pool (weight of 0.25).
	 */
	...
	/**
	 * This test submits jobs in two pools, poolA and poolB. None of the jobs in
	 * poolA have maps, but this should not affect their reduce share.
	 */
	public void testPoolWeightsWhenNoMaps() throws Exception
	{
		...
		/*
		 * assertEquals(0, info1.mapWeight, 0.01); assertEquals(1.0,
		 * info1.reduceWeight, 0.01); assertEquals(0, info2.mapWeight, 0.01);
		 * assertEquals(1.0, info2.reduceWeight, 0.01); assertEquals(1.0,
		 * info3.mapWeight, 0.01); assertEquals(1.0, info3.reduceWeight, 0.01);
		 */
	...
	public void testPoolMaxMapsReduces() throws Exception
	{
		...
		assertEquals(1, limited.getSchedulable(TaskType.REDUCE)
				.getRunningTasks());
		Pool defaultPool = scheduler.getPoolManager().getPool("default");
		assertEquals(2, defaultPool.getSchedulable(TaskType.MAP)
				.getRunningTasks());
	...
	/**
	 * This test starts by launching a job in the default pool that takes all
	 * the slots in the cluster. We then submit a job in a pool with min share
	 * of 2 maps and 1 reduce task. After the min share preemption timeout, this
	 * pool should be allowed to preempt tasks.
	 */
	public void testMinSharePreemption() throws Exception
	{
		...
		// tasks on tt1 and tt2 to ensure that the ones on tt2 get preempted
		// first.
		JobInProgress job1 = submitJob(JobStatus.RUNNING, 10, 10);
		checkAssignment("tt1", "attempt_test_0001_m_000000_0 on tt1");
		checkAssignment("tt1", "attempt_test_0001_r_000000_0 on tt1");
		...
		assertEquals(
				0,
				scheduler.tasksToPreempt(poolA.getMapSchedulable(),
						clock.getTime()));
		assertEquals(
				0,
				scheduler.tasksToPreempt(poolA.getReduceSchedulable(),
						clock.getTime()));
	 ...
	 * This test starts by launching a job in the default pool that takes all
	 * the slots in the cluster. We then submit a job in a pool with min share
	 * of 3 maps and 3 reduce tasks, but which only actually needs 1 map and 2
	 * reduces. We check that this pool does not prempt more than this many
	 * tasks despite its min share being higher.
	 */
	 ...
	 * type. Finally, we submit a third job, job 3 in pool3, which gets no
	 * slots. At this point the fair share of each pool will be 8/3 ~= 2.7
	 * slots. Pool 1 will be above its fair share, pool 2 will be below it but
	 * at half fair share, and pool 3 will be below half fair share. Therefore
	 * pool 3 should preempt a task (after a timeout) but pools 1 and 2
	 * shouldn't.
	 */
	public void testFairSharePreemption() throws Exception
	{
		...
		// Grab pools (they'll be created even though they're not in the alloc
		// file)
		...
		// assignment stage to ensure that the tasks from job1 on tt3 are the
		// ones
		...
		assertEquals(
				0,
				scheduler.tasksToPreempt(pool2.getMapSchedulable(),
						clock.getTime()));
		assertEquals(
				0,
				scheduler.tasksToPreempt(pool2.getReduceSchedulable(),
						clock.getTime()));
		assertEquals(
				0,
				scheduler.tasksToPreempt(pool3.getMapSchedulable(),
						clock.getTime()));
		assertEquals(
				0,
				scheduler.tasksToPreempt(pool3.getReduceSchedulable(),
						clock.getTime()));

		// Wait 2 more seconds, so that job 3 has now been in the system for
		// 61s.
		// Now pool 3 should be able to preempt 2 tasks (its share of 2.7
		// rounded
	 ...
	 * shares equal 2 slots to coexist (which makes the half-fair-share of each
	 * pool equal to 1 so that fair share preemption can kick in).
	 * 
	 * The test first starts job 1, which takes 3 map slots and 0 reduce slots,
	 * in pool 1. We then submit job 2 in pool 2, which takes 3 map slots and
	 * zero reduce slots. Finally, we submit a third job, job 3 in pool3, which
	 * gets no slots. At this point the fair share of each pool will be 6/3 = 2
	 * slots. Pool 1 and 2 will be above their fair share and pool 3 will be
	 * below half fair share. Therefore pool 3 should preempt tasks from both
	 * pool 1 & 2 (after a timeout) but pools 1 and 2 shouldn't.
	 */
	public void testFairSharePreemptionFromMultiplePools() throws Exception
	{
		...
		// Test that the tasks actually get preempted and we can assign new
		// ones.
	 ...
	 * a pool that has both a min share of 2 slots with a 60s timeout and a fair
	 * share timeout of 60s. After 60 seconds, this pool will be starved of both
	 * min share (2 slots of each type) and fair share (2 slots of each type),
	 * and we test that it does not kill more than 2 tasks of each type in
	 * total.
	 */
	...
	/**
	 * This is a copy of testMinAndFairSharePreemption that turns preemption off
	 * and verifies that no tasks get killed.
	 */
	...
	/**
	 * This is a copy of testMinAndFairSharePreemption that turns preemption on
	 * but also turns on mapred.fairscheduler.preemption.only.log (the "dry run"
	 * parameter for testing out preemption) and verifies that no tasks get
	 * killed.
	 */
	 ...
	 * with data on rack1.node2 and check that it doesn't get assigned on
	 * earlier nodes. A second job with no locality info should get assigned
	 * instead.
	 * 
	 * TaskTracker names in this test map to nodes as follows: - tt1 =
	 * rack1.node1 - tt2 = rack1.node2 - tt3 = rack2.node1 - tt4 = rack2.node2
	 */
	...
	/**
	 * This test submits a job and causes it to exceed its node-level delay, and
	 * thus to go on to launch a rack-local task. We submit one job with data on
	 * rack2.node4 and check that it does not get assigned on any of the other
	...
	public void testDelaySchedulingAtRackLevel() throws IOException
	{
		...
		// Advance time by 11 seconds to put us past the 10-second locality
		// delay
		advanceTime(11000);
	 ...
	 * then its rack-level delay. It should then launch tasks off-rack. However,
	 * once the job gets a rack-local slot it should stay in-rack, and once it
	 * gets a node-local slot it should stay in-node. For simplicity, we don't
	...
	public void testDelaySchedulingOffRack() throws IOException
	{
		...
		// Advance time by 11 seconds to put us past the 10-sec node locality
		// delay
		advanceTime(11000);
		...
		// Advance time by 11 seconds to put us past the 10-sec rack locality
		// delay
		advanceTime(11000);
		...
		// Check that delay scheduling info is set so we are looking for
		// node-local
		// tasks at this point
		assertEquals(info1.lastMapLocalityLevel, LocalityLevel.NODE);
		assertEquals(info1.timeWaitedForLocalMap, 0);
	...
	/**
	 * This test submits two jobs with 4 maps and 3 reduces in total to a 4-node
	 * cluster. Although the cluster has 2 map slots and 2 reduce slots per
	 * node, it should only launch one map and one reduce on each node to
	 * balance the load. We check that this happens even if assignMultiple is
	 * set to true so the scheduler has the opportunity to launch multiple tasks
	 * per heartbeat.
	 */
	public void testAssignMultipleWithUnderloadedCluster() throws IOException
	{
		...
		// Assign tasks and check that at most one map and one reduce slot is
		// used
	...
	/**
	 * This test submits four jobs in the default pool, which is set to FIFO
	 * mode: - job1, with 1 map and 1 reduce - job2, with 3 maps and 3 reduces -
	 * job3, with 1 map, 1 reduce, and priority set to HIGH - job4, with 3 maps
	 * and 3 reduces
	 * 
	...
	public void testFifoPool() throws Exception
	{
		...
		// Assign tasks and check that they're given first to job3 (because it
		// is
		// high priority), then to job1, then to job2.
		checkAssignment("tt1", "attempt_test_0003_m_000000_0 on tt1");
		checkAssignment("tt1", "attempt_test_0003_r_000000_0 on tt1");
	...
	/**
	 * This test submits 2 large jobs each to 2 pools, which are both set to
	 * FIFO mode through the global defaultPoolSchedulingMode setting. We check
	 * that the scheduler assigns tasks only to the first job within each pool,
	 * but alternates between the pools to give each pool a fair share.
	 */
	...
	/**
	 * This test submits 2 large jobs each to 2 pools, one of which is set to
	 * FIFO mode through the global defaultPoolSchedulingMode setting, and one
	 * of which is set to fair mode. We check that the scheduler assigns tasks
	 * only to the first job in the FIFO pool but to both jobs in the fair
	 * sharing pool.
	 */
	...
	/**
	 * This test uses the mapred.fairscheduler.pool property to assign jobs to
	 * pools.
	 */
	public void testPoolAssignment() throws Exception
	{
		...
		// Submit a job to the default pool. All specifications take default
		// values.
		...
		// Submit a job to poolA by specifying pool and not
		...
		JobInProgress job4 = new FakeJobInProgress(jobConf2,
				taskTrackerManager, null, UtilsForTests.getJobTracker());
			...
			String... expectedTasks) throws IOException
	{
		...
		for (int i = 0; i < tasks.size(); i++)
			assertEquals("assignment " + i, expectedTasks[i], tasks.get(i)
					.toString());
	}
	...
	private void verifyPoolMetrics()
	{
		...
		{
			assertEquals(
					scheduler.getPoolSchedulables(TaskType.MAP).size() * 2,
					records.size());
		}
		catch (Error e)
		{
		...
		for (PoolSchedulable pool : poolScheds)
		{
			...
			OutputRecord metrics = byPoolAndType.get(poolName + "_"
					+ pool.getTaskType().toString());
	...
	private void verifyJobMetrics()
	{
		...
		for (Map.Entry<JobInProgress, JobInfo> entry : scheduler.infos
				.entrySet())
		{
	 ...
	 */
	private void verifySchedulableMetrics(Schedulable sched,
			OutputRecord metrics)
	{
		...
		assertEquals(sched.getFairShare(), metrics.getMetric("fairShare")
				.doubleValue(), .001);
		assertEquals(sched.getWeight(), metrics.getMetric("weight")
				.doubleValue(), .001);
	}
	...
	public void testFairSharePreemptionWithShortTimeout() throws Exception
	{
		...
		assertEquals(
				0,
				scheduler.tasksToPreempt(pool1.getMapSchedulable(),
						clock.getTime()));
		assertEquals(
				0,
				scheduler.tasksToPreempt(pool1.getReduceSchedulable(),
						clock.getTime()));
		assertEquals(
				2,
				scheduler.tasksToPreempt(defaultPool.getMapSchedulable(),
						clock.getTime()));
		assertEquals(2, scheduler.tasksToPreempt(
				defaultPool.getReduceSchedulable(), clock.getTime()));
