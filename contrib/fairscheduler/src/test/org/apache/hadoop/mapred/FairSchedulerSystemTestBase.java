...
public abstract class FairSchedulerSystemTestBase
{
	...
	/**
	 * Check the fair scheduler servlet for good status code and smoke test for
	 * contents.
	 */
			...
			int jobTrackerInfoPort) throws Exception
	{
		...
		// Just to be sure, slurp the content and make sure it looks like the
		// scheduler
		...
		{
			String userGroups[] = UserGroupInformation.getCurrentUser()
					.getGroupNames();
	...
	{
		String jtURL = "http://localhost:" + jobTrackerInfoPort;
		URL url = new URL(jtURL + "/taskgraph?jobid=" + job.toString()
				+ "&type=map");
		HttpURLConnection connection = (HttpURLConnection) url.openConnection();
		connection.setRequestMethod("GET");
		connection.connect();
		...
		// Just to be sure, slurp the content and make sure it looks like a
		// graph
		String contents = slurpContents(connection);
	...
	protected String slurpContents(HttpURLConnection connection)
			throws Exception
	{
