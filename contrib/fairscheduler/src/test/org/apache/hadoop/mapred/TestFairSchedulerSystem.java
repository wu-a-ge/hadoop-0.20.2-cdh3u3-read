...
/**
 * System tests for the fair scheduler. These run slower than the mock-based
 * tests in TestFairScheduler but have a better chance of catching
 * synchronization bugs with the real JT.
 * 
 * This test suite will often be run inside JCarder in order to catch deadlock
 * bugs which have plagued the scheduler in the past - hence it is a bit of a
 * "grab-bag" of system tests, since it's important that they all run as part of
 * the same JVM instantiation.
 */
public class TestFairSchedulerSystem extends FairSchedulerSystemTestBase
{
	...
	/**
	 * Submit some concurrent sleep jobs, and visit the scheduler servlet while
	 * they're running.
	 */
	@Test
	public void testFairSchedulerSystem() throws Exception
	{
		...
		for (Future<Void> future : futures)
		{
			while (true)
			{
				...
				{
					System.err
							.println("No jobs running, not checking tasklog servlet");
					continue;
				}
				for (JobStatus j : jobs)
				{
					System.err.println("Checking task graph for "
							+ j.getJobID());
