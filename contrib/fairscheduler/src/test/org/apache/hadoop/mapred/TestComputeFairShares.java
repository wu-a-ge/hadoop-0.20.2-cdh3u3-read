...
public class TestComputeFairShares extends TestCase
{
	 ...
	 * only 3, and pool 3 gets the remaining 7 slots. Pool 4 also had a min
	 * share of 2 slots but this should not affect the outcome.
	 */
	...
	/**
	 * Weighted sharing test where pools 1 and 2 are now given lower demands
	 * than above. Pool 1 stops at 10 slots, leaving 35. If the remaining pools
	 * split this into a 1:1:0.5 ratio, they would get 14:14:7 slots
	 * respectively, but pool 2's demand is only 11, so it only gets 11. The
	 * remaining 2 pools split the 24 slots left into a 1:0.5 ratio, getting 16
	 * and 8 slots respectively.
	 */
	 ...
	 * above, pool 1 has a min share greater than its demand so it only gets its
	 * demand. Pool 3 has a min share of 15 even though its weight is very
	 * small, so it gets 15 slots. The remaining pools share the remaining 20
	 * slots equally, getting 10 each. Pool 3's min share of 5 slots doesn't
	 * affect this.
	 */
