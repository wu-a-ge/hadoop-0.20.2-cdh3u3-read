...
public class MiniDFSCluster
{
	...
	{
		this(0, conf, numDataNodes, format, true, true, null, racks, hosts,
				null);
	}
	...
	public MiniDFSCluster(int nameNodePort, Configuration conf,
			int numDataNodes, boolean format, boolean manageDfsDirs,
			StartupOption operation, String[] racks) throws IOException
	{
	...
	public MiniDFSCluster(int nameNodePort, Configuration conf,
			int numDataNodes, boolean format, boolean manageDfsDirs,
			StartupOption operation, String[] racks, long[] simulatedCapacities)
			throws IOException
	{
		this(nameNodePort, conf, numDataNodes, format, manageDfsDirs,
			...
			String hosts[], long[] simulatedCapacities) throws IOException
	{
		...
		FileSystem.setDefaultUri(conf,
				"hdfs://localhost:" + Integer.toString(nameNodePort));
		conf.set("dfs.http.address", "127.0.0.1:0");
		if (manageNameDfsDirs)
		{
			conf.set("dfs.name.dir", new File(base_dir, "name1").getPath()
					+ "," + new File(base_dir, "name2").getPath());
			conf.set("fs.checkpoint.dir",
					new File(base_dir, "namesecondary1").getPath() + ","
		...
		if (format)
		{
			...
			{
				throw new IOException("Cannot remove data directory: "
						+ data_dir);
			}
	...
	public void waitClusterUp()
	{
		if (numDataNodes > 0)
		{
			while (!isClusterUp())
			{
				...
				{
					System.err
							.println("Waiting for the Mini HDFS Cluster to start...");
					Thread.sleep(1000);
				}
			...
			throws IOException
	{
		...
		if (racks != null && numDataNodes > racks.length)
		{
			throw new IllegalArgumentException("The length of racks ["
					+ racks.length + "] is less than the number of datanodes ["
					+ numDataNodes + "].");
		}
		if (hosts != null && numDataNodes > hosts.length)
		{
			throw new IllegalArgumentException("The length of hosts ["
					+ hosts.length + "] is less than the number of datanodes ["
					+ numDataNodes + "].");
		}
				...
				&& numDataNodes > simulatedCapacities.length)
		{
			throw new IllegalArgumentException(
					"The length of simulatedCapacities ["
							+ simulatedCapacities.length
							+ "] is less than the number of datanodes ["
							+ numDataNodes + "].");
		}
		...
		for (int i = curDatanodesNum; i < curDatanodesNum + numDataNodes; i++)
		{
			...
			if (manageDfsDirs)
			{
				...
				if (!dir1.isDirectory() || !dir2.isDirectory())
				{
					throw new IOException(
							"Mkdirs failed to create directory for DataNode "
									+ i + ": " + dir1 + " or " + dir2);
				}
				dnConf.set(DataNode.DATA_DIR_KEY,
						dir1.getPath() + "," + dir2.getPath());
				conf.set(DataNode.DATA_DIR_KEY,
						dnConf.get(DataNode.DATA_DIR_KEY));
			}
			if (simulatedCapacities != null)
			{
			...
			System.out.println("Starting DataNode " + i
			...
			if (racks != null)
			{
				...
				System.out.println("Adding node with hostname : " + name
			...
			if (hosts != null)
			{
				NetUtils.addStaticResolution(hosts[i - curDatanodesNum],
						"localhost");
			}
			...
			if (racks != null)
			{
				...
				System.out.println("Adding node with IP:port : " + ipAddr + ":"
						+ port + " to rack " + racks[i - curDatanodesNum]);
	...
	public void finalizeCluster(Configuration conf) throws Exception
	{
		...
		ToolRunner.run(new DFSAdmin(conf), new String[] {
			"-finalizeUpgrade"
		});
	}
			...
			throws IOException
	{
		...
		while (true)
		{
			...
			{
				failedCount++;
				// Cached RPC connection to namenode, if any, is expected to
				// fail once
				if (failedCount > 1)
				{
	...
	public synchronized DataNodeProperties stopDataNode(int i)
	{
		...
		System.out.println("MiniDFSCluster Stopping DataNode "
				+ dn.dnRegistration.getName() + " from a total of "
	 ...
	 * @throws IOException
	 */
	public boolean restartDataNode(DataNodeProperties dnprop)
			throws IOException
	{
			...
			boolean keepPort) throws IOException
	{
		...
		dataNodes.add(new DataNodeProperties(DataNode
				.createDataNode(args, conf), newconf, args));
		numDataNodes++;
		return true;
	}
			...
			throws IOException, InterruptedException
	{
		final UserGroupInformation ugi = UserGroupInformation
				.createUserForTesting(username, groups);
			...
			throws InterruptedException
	{
		...
		return dataNodes.get(dataNodeIndex).datanode.getFSDataset()
				.getBlockReport();
	}
	 ...
	 * 
	 */
	public void injectBlocks(int dataNodeIndex, Block[] blocksToInject)
			throws IOException
	{
		if (dataNodeIndex < 0 || dataNodeIndex > dataNodes.size())
		{
		...
		FSDatasetInterface dataSet = dataNodes.get(dataNodeIndex).datanode
				.getFSDataset();
