...
public class DFSTestUtil
{
	 ...
	 */
	void createFiles(FileSystem fs, String topdir, short replicationFactor)
			throws IOException
	{
		...
		for (int idx = 0; idx < nFiles; idx++)
		{
			createFile(fs, new Path(root, files[idx].getName()),
					files[idx].getSize(), replicationFactor,
					files[idx].getSeed());
		}
	}
	...
	public static void waitForDatanodeStatus(FSNamesystem ns, int expectedLive,
			int expectedDead, long expectedVolFails,
			long expectedTotalCapacity, long timeout)
			throws InterruptedException, TimeoutException
	{
		...
		while ((expectedLive != live.size() || expectedDead != dead.size()
				|| expectedTotalCapacity != currTotalCapacity || expectedVolFails != volFails)
				&& count < ATTEMPTS);
		...
		if (count == ATTEMPTS)
		{
			throw new TimeoutException("Timed out waiting for capacity."
					...
					+ " Total capacity = " + currTotalCapacity + " Expected = "
					+ expectedTotalCapacity + " Vol Fails = " + volFails
					+ " Expected = " + expectedVolFails);
		}
	}
			...
			for (int j = 0; j < locs.length; j++)
			{
				...
				if (loc.length != replFactor)
				{
					System.out.println("File " + fileName
							+ " has replication factor " + loc.length);
					good = false;
					try
					{
						System.out
								.println("Waiting for replication factor to drain");
						Thread.sleep(100);
					}
	...
	public static Block getFirstBlock(FileSystem fs, Path path)
			throws IOException
	{
		DFSDataInputStream in = (DFSDataInputStream) ((DistributedFileSystem) fs)
				.open(path);
		in.readByte();
	...
	public static Token<BlockTokenIdentifier> getAccessToken(
			FSDataOutputStream out)
	{
		return ((DFSClient.DFSOutputStream) out.getWrappedStream())
				.getAccessToken();
	}
	...
	/**
	 * // TODO: JGH Reformat this damn code * Get a FileSystem instance as
	 * specified user in a doAs block.
	 */
