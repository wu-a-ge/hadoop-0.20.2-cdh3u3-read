...
public class SimulatedFSDataset implements FSConstants, FSDatasetInterface,
		Configurable
{
	...
	{ // information about a single block
		...
		BInfo(Block b, boolean forWriting) throws IOException
		{
			...
			if (!storage.alloc(theBlock.getNumBytes()))
		...
		synchronized SimulatedInputStream getIStream() throws IOException
		{
			if (!finalized)
			{
				...
				return new SimulatedInputStream(oStream.getLength(),
						DEFAULT_DATABYTE);
			}
			else
			{
	...
	{ // real construction when setConf called.. Uggg
	}
	...
	{
		conf = iconf;
		storageId = conf.get("StorageId",
				"unknownStorageId" + new Random().nextInt());
		registerMBean(storageId);
		storage = new SimulatedStorage(conf.getLong(CONFIG_PROPERTY_CAPACITY,
				DEFAULT_CAPACITY));
	...
	private synchronized void finalizeBlockInternal(Block b,
			boolean refinalizeOk) throws IOException
	{
	...
	public synchronized BlockWriteStreams writeToBlock(Block b,
			boolean isRecovery, boolean isReplicationRequest)
			throws IOException
	{
		if (isValidBlock(b))
		{
	...
	public synchronized long getChannelPosition(Block b,
			BlockWriteStreams stream) throws IOException
	{
		BInfo binfo = blockMap.get(b);
		if (binfo == null)
		{
	...
	public synchronized void setChannelPosition(Block b,
			BlockWriteStreams stream, long dataOffset, long ckOffset)
			throws IOException
	{
		BInfo binfo = blockMap.get(b);
		if (binfo == null)
	 ...
	 * "hadoop:service=DataNode,name=FSDatasetState-<storageid>" We use storage
	 * id for MBean name since a minicluster within a single Java VM may have
	...
	@Override
	public BlockLocalPathInfo getBlockLocalPathInfo(Block blk)
			throws IOException
	{
