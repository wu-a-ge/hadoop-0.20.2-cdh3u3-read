...
public class MiniMRCluster
{
	...
	public class JobTrackerRunner implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				final String id = new SimpleDateFormat("yyyyMMddHHmmssSSS")
						.format(new Date());
				if (ugi == null)
				{
				...
				{
					public JobTracker run() throws InterruptedException,
							IOException
					{
	...
	class TaskTrackerRunner implements Runnable
	{
		...
		TaskTrackerRunner(int trackerId, int numDir, String hostname,
				JobConf cfg) throws IOException
		{
			...
			conf.set("mapred.task.tracker.report.address", "127.0.0.1:"
					+ taskTrackerPort);
			File localDirBase = new File(conf.get("mapred.local.dir"))
					.getAbsoluteFile();
			localDirBase.mkdirs();
			StringBuffer localPath = new StringBuffer();
			for (int i = 0; i < numDir; ++i)
			{
				File ttDir = new File(localDirBase, Integer.toString(trackerId)
						+ "_" + i);
				if (!ttDir.mkdirs())
				{
			...
			try
			{
				...
				{
					public TaskTracker run() throws InterruptedException,
							IOException
					{
	...
	public void waitUntilIdle()
	{
		...
		try
		{
			...
			while (status.getTaskTrackers() + numTrackerToExclude < taskTrackerList
					.size())
			{
	...
	{
		for (Iterator<TaskTrackerRunner> itr = taskTrackerList.iterator(); itr
				.hasNext();)
		{
			TaskTrackerRunner runner = itr.next();
			while (!runner.isDead
					&& (!runner.isInitialized || !runner.tt.isIdle()))
			{
			...
			int jobTrackerPort, int jobTrackerInfoPort, UserGroupInformation ugi)
	{
		...
		result.set("mapred.job.tracker.http.address", "127.0.0.1:"
				+ jobTrackerInfoPort);
	...
	public MiniMRCluster(int jobTrackerPort, int taskTrackerPort,
			int numTaskTrackers, String namenode, int numDir)
			throws IOException
	{
	...
	{
		return jobTracker.getJobTracker()
				.getTaskCompletionEvents(id, from, max);
	}
			...
			int index, JobID jobId, int max) throws IOException
	{
		...
		TaskAttemptID dummy = new TaskAttemptID(jtId, jobId.getId(), false, 0,
				0);
	...
	{
		return jobTracker.getJobTracker().recoveryManager
	...
	int getTaskTrackerID(String trackerName)
	{
		...
		{
			if (taskTrackerList.get(id).getTaskTracker().getName()
					.equals(trackerName))
			{
				return id;
			}
