...
public class UtilsForTests
{
	...
	static void configureWaitingJobConf(JobConf jobConf, Path inDir,
			Path outputPath, int numMaps, int numRed, String jobName,
			String mapSignalFilename, String redSignalFilename)
			throws IOException
	{
	...
	static class WaitingMapper extends MapReduceBase implements
			Mapper<WritableComparable, Writable, WritableComparable, Writable>
	{
		...
		/**
		 * Checks if the map task needs to wait. By default all the maps will
		 * wait. This method needs to be overridden to make a custom waiting
		 * mapper.
		 */
		...
		/**
		 * Returns a signal file on which the map task should wait. By default
		 * all the maps wait on a single file passed as
		 * test.mapred.map.waiting.target. This method needs to be overridden to
		 * make a custom waiting mapper
		 */
			...
			if (shouldWait(id))
			{
				if (fs != null)
				{
					while (!fs.exists(getSignalFile(id)))
					{
						...
						{
							System.out
									.println("Interrupted while the map was waiting for "
											+ " the signal.");
							break;
						}
					}
		...
		public void configure(JobConf conf)
		{
			...
			{
				System.out
						.println("Got an exception while obtaining the filesystem");
			}
		}
	}
				...
				Reporter reporter) throws IOException
		{
			if (fs != null)
			{
				while (!fs.exists(signal))
				{
					...
					{
						System.out
								.println("Interrupted while the map was waiting for the"
										+ " signal.");
						break;
					}
				}
	...
	{
		return isMap ? "test.mapred.map.waiting.target"
	...
	static void signalTasks(MiniDFSCluster dfs, FileSystem fileSys,
			String mapSignalFile, String reduceSignalFile, int replication)
			throws IOException
	{
		writeFile(dfs.getNameNode(), fileSys.getConf(),
				new Path(mapSignalFile), (short) replication);
		writeFile(dfs.getNameNode(), fileSys.getConf(), new Path(
				reduceSignalFile), (short) replication);
	}
	 ...
	 */
	static void signalTasks(MiniDFSCluster dfs, FileSystem fileSys,
			boolean isMap, String mapSignalFile, String reduceSignalFile)
			throws IOException
	{
		// signal the maps to complete
		writeFile(dfs.getNameNode(), fileSys.getConf(), isMap ? new Path(
				mapSignalFile) : new Path(reduceSignalFile), (short) 1);
	}
			...
			short replication) throws IOException
	{
		...
		SequenceFile.Writer writer = SequenceFile.createWriter(fileSys, conf,
				name, BytesWritable.class, BytesWritable.class,
	 ...
	 * 
	 * @param dfs
	 *            FileSystem Local File System where file needs to be picked
	 * @param URIPATH
	 *            Path dfs path where file needs to be copied
	 * @param permission
	 *            FsPermission File permission
	 * @return returns the DataOutputStream
	 */
	public static DataOutputStream createTmpFileDFS(FileSystem dfs,
			Path URIPATH, FsPermission permission, String input)
			throws Exception
	{
	 ...
	 * 
	 * @param taskTrackerLong
	 *            String The long format of the tasktracker string
	 * @return String The FQDN of the tasktracker
	 * @throws Exception
	 */
	public static String getFQDNofTT(String taskTrackerLong) throws Exception
	{
		// Getting the exact FQDN of the tasktracker from the tasktracker
		// string.
	 ...
	 *            to setup
	 * @param localAlloc
	 *            local allocator to use
	 * @param dirs
	 *            to use for local storage
	 * @throws IOException
	 */
