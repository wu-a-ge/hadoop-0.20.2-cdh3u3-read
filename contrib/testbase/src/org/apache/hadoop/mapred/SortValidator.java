...
public class SortValidator extends Configured implements Tool
{
	...
	static private IntWritable deduceInputFile(JobConf job)
	{
		...
		return (inputFile.getParent().equals(inputPaths[0])) ? sortInput
				: sortOutput;
	}
	...
	static private byte[] pair(BytesWritable a, BytesWritable b)
	{
		...
		System.arraycopy(b.getBytes(), 0, pairData, a.getLength(),
				b.getLength());
		return pairData;
	}
	...
	public static class RecordStatsChecker
	{
				...
				Mapper<WritableComparable, Writable, IntWritable, RecordStatsWritable>
		{
					...
					try
					{
						...
						partition = Integer.valueOf(
								inputFile.substring(inputFile
										.lastIndexOf("part") + 5)).intValue();
						noSortReducers = job.getInt(
								"sortvalidate.sort.reduce.tasks", -1);
					}
					catch (Exception e)
					{
					...
					Reporter reporter) throws IOException
			{
				...
				if (this.key == sortOutput)
				{
					...
					int keyPartition = partitioner.getPartition(key, value,
							noSortReducers);
				...
				int keyValueChecksum = (WritableComparator.hashBytes(keyBytes,
						keyBytesLen) ^ WritableComparator.hashBytes(valueBytes,
						valueBytesLen));
				...
				Reducer<IntWritable, RecordStatsWritable, IntWritable, RecordStatsWritable>
		{
					...
					Reporter reporter) throws IOException
			{
				...
				output.collect(key,
						new RecordStatsWritable(bytes, records, xor));
			}
		}
		...
		static void checkRecords(Configuration defaults, Path sortInput,
				Path sortOutput) throws IOException
		{
			...
			int noSortReduceTasks = outputfs.listStatus(sortOutput,
					sortPathsFilter).length;
			...
			{
				throw new IOException("(" + v1.getBytes() + ", "
						+ v1.getRecords() + ", " + v1.getChecksum() + ") v/s ("
	...
	public static class RecordChecker
	{
				...
				Reducer<BytesWritable, IntWritable, BytesWritable, IntWritable>
		{
					...
					Reporter reporter) throws IOException
			{
				...
				while (values.hasNext())
				{
					...
					{
						throw new IOException("Invalid 'value' of "
								+ count.get() + " for (key,value): "
								+ key.toString());
					}
				}
				...
				if (ones != twos)
				{
					throw new IOException("Illegal ('one', 'two'): (" + ones
							+ ", " + twos + ") for (key, value): "
							+ key.toString());
				}
			}
		...
		static void checkRecords(Configuration defaults, int noMaps,
				int noReduces, Path sortInput, Path sortOutput)
				throws IOException
		{
			...
			if (noReduces == -1)
			{
				...
				String sortReduces = jobConf
						.get("test.sortvalidate.reduces_per_host");
				if (sortReduces != null)
				{
			...
			System.out.println("\nSortValidator.RecordChecker: Running on "
					...
					+ " nodes to validate sort from " + inputPaths[0] + ", "
					+ inputPaths[1] + " into "
	...
	{
		int res = ToolRunner
				.run(new Configuration(), new SortValidator(), args);
		System.exit(res);
	}
}
