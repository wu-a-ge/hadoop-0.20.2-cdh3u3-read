		...
		InputFormat<NullWritable, NullWritable>
{
	...
	private void initialize() throws IOException
	{
		...
		writeFile(new Path(signalFileDir, REDUCE_SIGFILE_PREFIX
				+ reducesFinished));
	}
			...
			throws IOException
	{
		...
		Path oldSignalFile = new Path(signalFileDir, PREFIX
				+ String.valueOf(tasksFinished));
		Path newSignalFile = new Path(signalFileDir, PREFIX
	 ...
	 * started running. This currently uses a jip object and directly uses its
	 * api to determine the number of tasks running.
	 * 
	 * <p>
	 * 
	...
	static void waitTillNTasksStartRunning(JobInProgress jip, boolean isMap,
			int noOfTasksToBeRunning) throws InterruptedException
	{
		int numTasks = 0;
	...
	static void assertNumTasksRunning(JobInProgress jip, boolean isMap,
			int noOfTasksToBeRunning) throws Exception
	{
	 ...
	 * are finished. This currently uses a jip object and directly uses its api
	 * to determine the number of tasks finished.
	 * 
	 * <p>
	 * 
	 * TODO: It should eventually use a JobID and then get the information from
	...
	static void waitTillNTotalTasksFinish(JobInProgress jip, boolean isMap,
			int noOfTasksToFinish) throws InterruptedException
	{
		int noOfTasksAlreadyFinished = 0;
		while (noOfTasksAlreadyFinished < noOfTasksToFinish)
		{
			Thread.sleep(1000);
			noOfTasksAlreadyFinished = (isMap ? jip.finishedMaps() : jip
					.finishedReduces());
			...
			throws IOException
	{
		...
		while (!signalType.equals("MAPS") || taskNumber + 1 > noOfTasks)
		{
			LOG.info("Signal type found : " + signalType
					+ " .Number of tasks to be finished by this signal : "
					+ noOfTasks + " . My id : " + taskNumber);
	...
	{
		JobConf conf = getControlledMapReduceJobConf(getConf(), numMapper,
				numReducer);
	...
	private JobConf getControlledMapReduceJobConf(Configuration clusterConf,
			int numMapper, int numReducer) throws IOException
	{
		setConf(clusterConf);
		initialize();
	...
	static class ControlledMapReduceJobRunner extends Thread
	{
		...
		public void run()
		{
			...
			try
			{
				...
				int ret = ToolRunner.run(this.conf, job, new String[] {
		...
		{
			return new ControlledMapReduceJobRunner(conf, numMappers,
					numReducers);
		}
	}
}
